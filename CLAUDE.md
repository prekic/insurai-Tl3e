...
1. **Merge PR #351**: The Ray Sigorta QA bug fixes (PR `fix(extraction): fix 8 QA bugs from Ray Sigorta commercial kasko`) are pushed on branch `claude/load-project-context-BkEVh`. CI should be green (617 tests). Merge to main when ready.
2. **Upload Ray Sigorta PDF fixture (deferred)**: The original Ray Sigorta PDF `KRK_35_VD_458_Kasko_Police_32630901_3.pdf` was NOT uploaded to the repo. If obtained, drop into `policies/` (committed, runs in CI) or `test-data/` (gitignored, local-only). Then add a fixture entry to `src/lib/ai/__tests__/qa-pdf-golden.test.ts` with expected premium 755.21 TL, 17 DAHİL + 10 HARİÇ coverages, vehicle IVECO/KAMYON 80-12 1997, plate 35 VD 458.
3. **Monitor Calibration Post-Pilot**: We forced Pilot threshold writing by bypassing the 50-sample `calibrate-grade-thresholds.ts` script limiter. Monitor these p90/p75 thresholds (A: 93, B: 85, C/D: 60) for skew as larger batches enter.
4. **Phase E Production Monitoring**: The KASKO pilot pipeline is hardened and deduplicated. We are now ready to commence Phase E operations.
...
- **Production Readiness**: 10/10 (16,155+ tests + 63 new QA regression tests, 0 lint errors, 0 TS errors, 100% test pass rate for modified suites, PWA support, server hardening, Dynamic AI Rules, DB AI Prompts, UI Evidence Tracking, Safety Governance, PWA icons)
- **Last Updated**: April 16, 2026 (Ray Sigorta Commercial Kasko QA session. Fixed 8 extraction/interpretation bugs surfaced by QA review of a 1997 IVECO fleet truck kasko policy. Hardened Turkish İ (U+0130) regex handling across premium extraction, sigorta bedeli, and DAHİL/HARİÇ detection — JS `/i` flag does simple case folding and does NOT match `PRİM` against `prim`. Added `included` field to extraction schema + AI prompt + ExtractedCoverage + StructuredPolicyData; table parser now preserves HARİÇ coverages with `included: false` instead of dropping them. Added historical-policy threshold (>2yr expired → "Historical Policy — For Reference Only"). Filtered "Unresolved relationship" internal warnings out of aiInsights. Deductible assignment now uses max across coverages. Added 63 new regression tests (38 unit + 25 golden-against-real-PDF) loading 4 committed Turkish kasko PDFs from `policies/`. PR #351 on branch `claude/load-project-context-BkEVh`.)
