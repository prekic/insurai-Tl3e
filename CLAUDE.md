...
- **Last Updated**: May 3, 2026 (8 PRs this session: #456 Option-A unverified-gate relaxation — share/download always allowed; #457 Output Stability gate fix — fixture/shape/regex breadth (3 bugs); #458 handoff doc correction — May-2 verdict was false-positive; #459 migration 058 AS+ extraction; #460 same-day rollback (059) + compact form (060) after prompt-bloat regression; #461 cost controls × 4 — path filter on smoke-kasko, `STABILITY_RUNS` 5→3, `ocr_cache` table + service + route wiring, smoke-kasko Haiku 4.5 + extraction-model allowlist; #462 OCR cache key fix — `cacheKey` field replaces non-deterministic `sha256(documentBase64)` because pdf-lib's `save()` is non-deterministic across Node processes; #463 smoke-kasko timeout 10→15 min. Migrations 058-061 applied to production. **OCR cache verified working: 7 rows, 7 hits after two smoke runs**. New ADR-025 documents the OCR cache infrastructure. Cost-projection: ~$3-4/day burn → ~$0.50-1.00/day steady-state (~70-80% reduction). Operator pending: restore `app_settings.ai.temperature` from 0 → 0.1 (still inherited from May-2 Test B).)
