...
1. **Apply Migration 049** (named-deductible attribution rules) to production Supabase if not already done — it shipped Sprint 1 but the Apr-30 audit showed `cd=0` across all 13 corpus PDFs, suggesting either the migration didn't apply OR the corpus genuinely lacks named-percentage scenarios. Verify both possibilities. The reviewer's exact Anadolu Birleşik Kasko fixture (with %35/%80 scenarios) is NOT in the corpus — manually upload it to confirm migration 049 is firing.
2. **Smoke automation is now CI-gated.** Every push to `main` runs `scripts/smoke-kasko.ts` against production via `.github/workflows/smoke-kasko.yml` (4 fixtures, ~3 min, 80% pass-rate threshold). Before shipping extraction-pipeline changes, expect this to surface regressions automatically.
3. **Three audit tiers** for post-Sprint verification: (a) smoke (CI), (b) `scripts/audit-all-policies.ts` (~14 PDFs, vehicle-only, ~25 min), (c) `scripts/audit-end-to-end.ts` (~13 PDFs, full Sprint 2 surface, ~30 min). See ADR-022.
4. **Sprint 3 candidates** (reviewer's polish/UX tail): #10 premium split detail, #12 coverage transfer context, #13 Special Provisions panel, #14 AS+ servis network callout, #15 output stability tuning.
...
7. **Sandbox `git push` does NOT trigger Railway** (gotcha #22) — use `mcp__github__merge_pull_request` to fire the deploy webhook.
...
- **Last Updated**: April 30, 2026 (Sprint 0+1+2 reviewer-fix pass: shipped 14 PRs across structured-PostgrestError logging plumbing (#393-398 + migrations 046/047), 4-PR smoke automation infrastructure (#399-410 — fixtures, SSE-aware client, chunked OCR, cross-insurer leak guard, 5xx retry, GitHub Actions workflow), 3 P0 reviewer hard-blockers (#407-409 + migration 049), and 5 P1 high-value-accuracy fixes (#412-416 + migrations 050/051/052). Three audit tiers introduced (smoke + 14-PDF vehicle audit + 13-PDF Sprint 2 e2e — ADR-022). Extraction prompts now mandate vehicle fields, demand specific Anadolu add-ons, scope exclusions to labeled sections, and itemize conditional deductibles. UI surfaces bundle badges, rayiç-vs-fixed Bedeli detection, per-scenario severity-bucketed financial risks, and pre-filled Ask-Insurer answers. Migrations 048-052 manually applied to production Supabase prompt_templates. Smoke verified 4/4 vehicle pass; e2e audit confirmed 10/13 bundles + 7/13 with pre-filled answers + 3/13 with carve-outs surfaced.)
