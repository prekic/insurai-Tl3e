...
1. **Merge branch `claude/load-project-context-d2ylb` → main**: 4 commits pushed (`26e49ee`, `2f3f0b7`, `33c2c65`, `b1e6072`). Recommended squash-merge title: `feat(pilot,extraction,admin): Phase E scale-up + 7 deferred QA fixes + email resolver + 3 runbooks`. Release-please will pick the dominant `feat` type. No PR opened yet per user request.
2. **Ray Sigorta PDF fixture** ✅ **DONE (Apr 18 follow-up)**: Committed at `policies/KRK_35 VD 458 Kasko Police_32630901_3.pdf` (239 KB, 6 pages) and wired into `src/lib/ai/__tests__/qa-pdf-golden.test.ts` via a new `requiresOcr: true` branch on the `PdfFixture` shape. The PDF is scanned/image-only (0 extractable text via `pdf-parse` and `pdfjs-dist`), so the regex-layer asserts are skipped for this fixture and replaced with (a) an OCR-fallback signal check (`text.length < 200`) and (b) canonical-value guards (premium 755.21 TL, 17 DAHİL + 10 HARİÇ, IVECO/KAMYON 80-12 1997, plate 35 VD 458) for the future OCR-backed test. Current suite: 27 tests (was 25; +2). Production must route this PDF through GCP Document AI (gotcha #61).
3. **Run date-corruption audit**: New runbook `docs/runbooks/05-date-corruption-audit.md` has the SQL + repair commands for V8 DD.MM.YYYY bug (gotcha #52). Production code is fixed; historical rows may still be corrupted. Needs prod DB access.
...
5. **MIN_SAMPLE_SIZE flip auto-handled**: The calibration script now has `--auto-production` mode. When pilot samples hit ≥ 50 AND last audit shows `forced: true`, it auto-escalates. Cron-schedulable. No manual flip needed.
6. **Email resolver opt-in**: New `POST /api/admin/app-users/resolve-emails` endpoint is live but disabled by default. Set `ENABLE_ADMIN_EMAIL_RESOLVER=true` in the server env after privacy review to enable email-paste mode in the Segments admin tab.
...
- **Last Updated**: April 18, 2026 (Phase E + Autonomous Follow-ups session, branch `claude/load-project-context-d2ylb`, 4 commits: `26e49ee` → `b1e6072`. Scope — (1) **7 deferred QA bugs** (Ray Sigorta P1/P2 #6 #7 #9 #10 #13 #14 #15) shipped; (2) **Phase E production scale-up** wired end-to-end: `rollout_percentage` now reaches the pilot gate via new `usePilotGateOptions` shape + shared `computeRolloutBucket()` helper; **Segments admin tab** created for reviewer management (UUID paste + opt-in email paste); **Phase E runbook** `04-phase-e-production-scaleup.md` authored with 25%→100% ladder; (3) **3 new runbooks**: Phase E (`04`), date-corruption audit (`05`), Phase F skeleton (`06`); (4) **Calibration `--auto-production` mode** — cron-safe probe that auto-escalates when pilot hits n≥50; (5) **Hygiene**: 6 pre-existing TrustworthinessUI test failures unblocked (banner copy aligned with PolicyCard), `[ConfidenceDiag]` docs corrected to reality (1 site not 5), migration 040 RLS comments rewritten to accurately describe intentional-open design; (6) **New gated endpoint** `POST /api/admin/app-users/resolve-emails` behind `ENABLE_ADMIN_EMAIL_RESOLVER=true` server env var pending privacy review; (7) **ADR-0004** documents the env-var-opt-in pattern for privacy-sensitive admin features.)
