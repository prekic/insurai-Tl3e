...
- **Last Updated**: April 12, 2026 (V8 DD.MM.YYYY day/month swap bug **FIXED** in production — commit `ed487ef`: 5 sites across `policy-extractor.ts`, `extraction-validator.ts`, `policy-utils.ts` now use `parseTurkishDate()` from `turkish-utils.ts`, 8 regression tests added. `.env` created with all keys except `PILOT_REVIEWER_USER_ID`. Phase C blocked on sandbox network — requires machine with outbound HTTPS.)
