...
1. **Merge branch `claude/load-project-context-RJ0Qd` → main**: 3 commits ahead of `origin/main` — `1ed2284` (Ray Sigorta scanned PDF fixture), `93c56bd` (audit-only mode for date-corruption script), `c73f877` (docs sync: gotcha #83 + handoff rewrite). Plus a post-audit commit adding the missing qa-pdf-golden entry and parser-drift warning. Suggested squash-merge title in the active PR-title section below. Release-please will pick the dominant `feat` type.
...
- **Last Updated**: April 18, 2026 (Date-Corruption Audit Tooling session, branch `claude/load-project-context-RJ0Qd`, 1 new commit `93c56bd` on top of `1ed2284` from the Ray Sigorta fixture drop. Scope — (1) **`scripts/backfill-date-bug.ts` rewritten** with safe-default `--audit-only` mode, explicit `--apply` gate with interactive confirm, shared `classifyRow()` classifier pinning runbook 05 §3 semantics, CSV report + applied-log output; (2) **12 unit tests** in `scripts/__tests__/backfill-date-bug.test.ts` covering CORRUPTED / OK / MANUAL_REVIEW / null-candidate cases; (3) **Runbook 05 §4.4 added** — Option D: Script-Assisted Audit & Repair; (4) **New gotcha #83** — read-only-by-default contract for prod-touching operator scripts. Unblocks Next Session Instruction #3 on the operator side — tooling ready, only missing is the operator running it locally with their `.env`.)
