...
1. **Self-audit layer (Phases 1-4) is shipped and code-complete.** Migrations 053 (`audit_judgements`), 054 (`Audit Judge - Kasko` prompt + 3 `app_settings` rows), and 055 (`audit_trend_snapshots`) are applied to production Supabase. Live policy uploads now fire-and-forget a per-typology audit-judge run; weekly trend tracking runs via `.github/workflows/audit-judge-trends.yml` — `schedule:` cron is **enabled** (Mondays 06:00 UTC, trends-only by default; pass `run_judge=true` via `workflow_dispatch` to also fire the LLM judge).
2. **Three deferred follow-ups from May 1 are now addressed by the May 2 PRs** (#425, #426, #427, #428). Phase-1 source review revealed two of the three handoff-stated root causes were already correct in source — real bugs were subtler: `cost_usd` null was a versioned-model-name lookup miss (Anthropic echoes `claude-sonnet-4-6-20251022` instead of the bare alias); admin notifications "not firing" was an opaque first-only suppression conflated with silent query errors; `normaliseInsurer()` lacked NFKC + whitespace pre-pass. All three shipped together; observability is now sufficient that the next genuine failure surfaces in Railway logs (escalated catches + reason-coded suppression info logs). Diagnostic for follow-up investigation: `npx tsx scripts/diagnose-audit-judge-observability.ts`.
3. **Apply Migration 049** (named-deductible attribution rules) to production Supabase if not already done — it shipped Sprint 1 but the Apr-30 audit showed `cd=0` across all 13 corpus PDFs, suggesting either the migration didn't apply OR the corpus genuinely lacks named-percentage scenarios. Verify both possibilities. The reviewer's Anadolu Birleşik Kasko fixture (which has a %80 commercial-use co-insurance scenario, not %35) is **now in the corpus** as `tests/fixtures/kasko/anadolu-birlesik-kasko.pdf` (PR #426). The next workflow run will exercise it end-to-end.
4. **Smoke automation is now CI-gated.** Every push to `main` runs `scripts/smoke-kasko.ts` against production via `.github/workflows/smoke-kasko.yml` (4 fixtures, ~3 min, 80% pass-rate threshold). Before shipping extraction-pipeline changes, expect this to surface regressions automatically.
5. **Three audit tiers** for post-Sprint verification: (a) smoke (CI), (b) `scripts/audit-all-policies.ts` (~14 PDFs, vehicle-only, ~25 min), (c) `scripts/audit-end-to-end.ts` (~13 PDFs, full Sprint 2 surface, ~30 min). See ADR-022. **Plus Phase 4 trend tracking** via `npm run audit:trends` (3 golden fixtures + Supabase snapshot diff vs last baseline).
6. **Sprint 3 candidates** (reviewer's polish/UX tail): #10 premium split detail, #12 coverage transfer context, #13 Special Provisions panel, #14 AS+ servis network callout, #15 output stability tuning.
...
8. **🚨 QA GATE BEFORE SHIPPING EXTRACTION/DISPLAY CHANGES 🚨**: Before claiming any fix complete that touches `aiConfidence`, `extractionIncomplete`, the display-mode gate, or vehicle/coverage extraction, run `npm run qa:extraction` and confirm the relevant check moved. See gotcha #102, runbook 07, ADR-020.
9. **Sandbox `git push` does NOT trigger Railway** (gotcha #22) — use `mcp__github__merge_pull_request` to fire the deploy webhook.
10. **Audit-layer May 1 follow-ups closed** (PRs #425, #426, #427, #428 merged May 2). (a) `cost_usd` null was caused by Anthropic echoing a versioned model name (`claude-sonnet-4-6-20251022`) that missed the bare-alias pricing key — fixed by billing against the operator-configured `judgeModel` instead of `response.model`, plus a `normaliseModelKey()` belt-and-suspenders that strips `-\d{6,}$` suffixes. (b) Admin notifications "not firing" turned out to be opaque suppression: the strict first-only guard returned a bare boolean that conflated correct duplicate-suppression with silent Supabase query errors — renamed to `evaluateNotificationDecision()` returning `{ notify, reason }` so the caller logs WHY each suppression happened. (c) `normaliseInsurer()` now applies `.normalize('NFKC').replace(/\s+/g, ' ')` BEFORE suffix-stripping so visually-equivalent insurer strings hash to the same typology.
...
- **Last Updated**: May 2, 2026 (Self-audit-layer follow-ups closed across 4 PRs (#425-#428) after the May-1 ship surfaced 3 observability gaps. PR #425: NFKC + whitespace pre-pass in `normaliseInsurer()` so visually-equivalent insurer strings hash to the same typology (gotcha #143). PR #426: promoted `policies/ANADOLU.PDF` to the golden corpus as `anadolu-birlesik-kasko` — verified via `scripts/probe-anadolu-birlesik.ts` to contain a %80 commercial-use co-insurance scenario (the reviewer-flagged April 30 case; %35 was misremembered). PR #427: cost_usd persistence fixed (bills against operator-configured `judgeModel` instead of Anthropic-echoed versioned name; `normaliseModelKey()` belt-and-suspenders strips `-\d{6,}$` suffixes); admin-notification suppression reasons now typed (`evaluateNotificationDecision()` returns `{ notify, reason }`) and logged at info level on every suppression; `notifyAuditQuality.catch` escalated warn→error. PR #428: `schedule:` cron enabled on `.github/workflows/audit-judge-trends.yml` (Mondays 06:00 UTC, trends-only by default) after manual run #4 confirmed calibration on all 4 fixtures including the new one. New diagnostic script: `npx tsx scripts/diagnose-audit-judge-observability.ts`.)
