...
3. **Database migrations** — all applied to production Supabase:
   ...
   - ✅ **Migration 043** (`benchmark_aging_days` + `benchmark_stale_days` in `app_settings`) — applied Apr 9, 2026
4. **Pilot Batch Ingestion**: Bulk upload the remaining `upload/real-kasko-pdf/` files into the web UI application.
5. **Execute Evaluation Backfill**: Run `npx tsx scripts/backfill-evaluation-scores.ts --apply` to generate database scores for all ingested policies.
6. **Calibrate Grade Thresholds**: Run `scripts/calibrate-grade-thresholds.ts` once 50+ scored policies exist to adjust the A/B/C/D grade cutoffs.
...
- **Last Updated**: April 9, 2026 (Migrations 042+043 applied to production; PR-A: market comparison test regex alignment; PR-B: server schema parity — nameTr + currency fix, strict-mode-validator extraction, cross-file parity test, CLAUDE.md gotchas #48/#49 updated)
