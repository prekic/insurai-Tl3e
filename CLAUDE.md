...
1. **Create PR for `insuraigemini20260417`**: Branch has 3 commits ahead of `main` (QA bug fixes, depreciation/non-OEM prompt expansion, console noise suppression). All 2,709 tests pass, 0 lint errors. Suggested PR title: `fix(extraction): deferred P1/P2 QA bugs, depreciation prompts, and test noise suppression`.
...
- **Production Readiness**: 10/10 (2,709+ tests across 55 suites, 0 lint errors, 0 TS errors, 100% test pass rate, PWA support, server hardening, Dynamic AI Rules, DB AI Prompts, UI Evidence Tracking, Safety Governance, PWA icons)
- **Last Updated**: April 19, 2026 (Deferred P1/P2 QA hardening session. Fixed NCD display, commercial vehicle suppression, locale mixing leaks, and confidence penalization for unresolved-relationship warnings. Expanded non-OEM parts detection with `eşdeğer`/`çıkma`/`muadil` pattern matching & risk-flag injection. Added depreciation prompt guidance for `Eskime Payı`/`Kıymet Artışı` on aged vehicles. Fixed dynamic PDF test for foreign-currency extraction. Added `vehicleUsage` and `insuredEntityType` to extraction schema and `AnalyzedPolicy` type. Suppressed 1,074 lines of `[PolicyExtractor]` diagnostic console noise in Vitest output while preserving `[ClauseResolver]` safety-guard warnings. Added OCR sidecar text file for Ray Sigorta PDF golden tests. Branch `insuraigemini20260417`, 3 commits ahead of main.)
