...
4. **🔴 Pilot Batch Ingestion — Phase A script ready, Phase B + C pending (as of Apr 11)**:
   ...
   - 🔴 **Phase B (user action, pending)**: Create `/home/user/insurai/.env` with `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `GCP_SERVICE_ACCOUNT_BASE64`, and `PILOT_REVIEWER_USER_ID` (real `auth.users` UUID — `policies.user_id` has NOT NULL FK). `.env` is in `.gitignore`.
