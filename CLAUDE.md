...
1. **Merge branch `claude/load-project-context-RJ0Qd` → main**: 4 named feature/docs commits ahead of `origin/main` — `1ed2284` (Ray Sigorta scanned PDF fixture), `93c56bd` (audit-only mode for date-corruption script), `c73f877` (docs sync: gotcha #83 + handoff rewrite), `0f6854d` (completeness-audit fixes: commit count, qa-pdf-golden inventory, parser-drift warning). This entry itself may land as an additional stabilization commit — authoritative count: `git rev-list origin/main..HEAD --count`. Suggested squash-merge title in the active PR-title section below. Release-please will pick the dominant `feat` type.
2. **Ray Sigorta PDF fixture** ✅ **DONE (Apr 18)**: Committed at `policies/KRK_35 VD 458 Kasko Police_32630901_3.pdf` (239 KB, 6 pages) and wired into `src/lib/ai/__tests__/qa-pdf-golden.test.ts` via a `requiresOcr: true` branch on the `PdfFixture` shape. The PDF is scanned/image-only (0 extractable text via `pdf-parse` and `pdfjs-dist`), so regex-layer asserts are skipped for this fixture and replaced with (a) an OCR-fallback signal check (`text.length < 200`) and (b) canonical-value guards (premium 755.21 TL, 17 DAHİL + 10 HARİÇ, IVECO/KAMYON 80-12 1997, plate 35 VD 458). Current suite: 27 tests (+2). Production must route this PDF through GCP Document AI (gotcha #61).
3. **Run date-corruption audit** (tooling now ready, still blocked on operator DB access): Enhanced script `scripts/backfill-date-bug.ts` defaults to `--audit-only` (read-only; emits 4-count summary + CSV report). Explicit `--apply` required for repair. Operator runs locally with `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` in `.env` — no credential handoff to Claude. See runbook `docs/runbooks/05-date-corruption-audit.md` §4.4 for the workflow. After `--apply`, run `scripts/backfill-evaluation-scores.ts` to rescore affected rows. 12-test unit suite for `classifyRow()` at `scripts/__tests__/backfill-date-bug.test.ts`.
4. **Pilot activation** (3 operator steps, runbook `docs/runbooks/04-phase-e-production-scaleup.md`): (a) apply migration 040 if not already; (b) assign reviewer UUIDs via new Segments admin tab; (c) enable `kasko_ai_extraction_pilot` flag (rollout_percentage = 0 for Phase D, 25 for E1 after 14-day clean soak, 100 for E2 after 7-day clean soak).
5. **MIN_SAMPLE_SIZE flip auto-handled**: The calibration script has `--auto-production` mode. When pilot samples hit ≥ 50 AND last audit shows `forced: true`, it auto-escalates. Cron-schedulable. No manual flip needed.
6. **Email resolver opt-in**: `POST /api/admin/app-users/resolve-emails` endpoint is live but disabled by default. Set `ENABLE_ADMIN_EMAIL_RESOLVER=true` in the server env after privacy review to enable email-paste mode in the Segments admin tab.
7. **🚨 TESTING PROTOCOL WARNING 🚨**: Never run full test suite (>10 min). Always isolated: `npx vitest run path/to/file.test.ts`.
...
73. **Pilot Gate Flag Shape — Polymorphic Back-Compat (Added Apr 18, 2026)**: `evaluateKaskoPilotGate()` in `src/lib/analysis/kasko-pilot-gate.ts` now accepts `Record<string, boolean | { enabled, rolloutPercentage }>` for `featureFlags`. Legacy callers passing boolean maps keep working — the gate normalizes `true` as `{enabled: true, rolloutPercentage: 100}`. Phase E consumers (via `usePilotGateOptions`) pass the object form so the gate can apply deterministic per-user bucketing. When adding new callers, prefer the object form. `PilotFeatureFlagValue` type is exported for type-sharing with hooks.

74. **Shared Rollout Hash — `src/lib/config/rollout-hash.ts` (Added Apr 18, 2026)**: `computeRolloutBucket(userId, flagKey)` is the canonical hash used by BOTH `ConfigurationService.isFeatureEnabled()` AND `evaluateKaskoPilotGate()`. Keeps bucket assignments consistent — a user bucketed in at 50% by one caller is bucketed in at 50% by any other caller. **Critical rule**: do NOT substitute a different hash algorithm (e.g. djb2) without a migration plan for users already live at partial rollout. The current 32-bit bitwise hash was extracted verbatim from `configuration-service.ts` to preserve existing bucket assignments.

75. **Opt-in Env Var for Privacy-Sensitive Admin Features (Added Apr 18, 2026)**: `POST /api/admin/app-users/resolve-emails` exposes `auth.users.email` to super-admins. It is gated by `ENABLE_ADMIN_EMAIL_RESOLVER=true` server env var (default disabled, returns 403 `RESOLVER_DISABLED`). Pattern: env var (not DB feature flag) because the decision is privacy-operational, should not be toggleable from the admin UI, and should require an explicit deploy-time decision. Follow this pattern for future privacy-sensitive admin features. See ADR-0004 for the full rationale.

76. **Migration 040 RLS is Intentionally Open (Corrected Apr 18, 2026)**: `user_segments` and `kasko_pilot_qa_records` policies are `USING (true) WITH CHECK (true)`. Migration comments historically claimed "service role only" but that was misleading — they've been rewritten to accurately name the policies `"Open read/write (API layer enforces auth)"` and explain the intent inline. Auth is enforced at the API layer (`requireSuperAdmin` on admin routes + `.eq('user_id', user.id)` filter in client hooks). **Do NOT tighten** these policies without also proxying `persistPilotQARecord()` and `usePilotGateOptions` through server-side authenticated endpoints — both currently use the anon key and will silently fail otherwise.

77. **Calibration `--auto-production` Probe (Added Apr 18, 2026)**: `scripts/calibrate-grade-thresholds.ts --auto-production` is a cron-safe mode that checks both (a) sample count ≥ 50 and (b) last audit record shows `forced: true` before auto-escalating to production mode. Exits 0 as a no-op when conditions aren't met. This replaces the manual "flip MIN_SAMPLE_SIZE 5→50 + run `--production --apply`" sequence that Phase D previously required. When scheduling in CI/cron, use `--auto-production --apply` (not `--production --apply`) — the probe guards against redundant writes when the audit already shows a production-mode run.

78. **TrustworthinessUI Banner Copy = `UNVERIFIED AI OUTPUT` (Fixed Apr 18, 2026)**: `PolicyDetailView.tsx:1105-1119` banner previously said only "AI output has not yet received human expert verification…" which drifted from the established UX label. Now prefixes with bold `"Unverified AI output"` (EN) / `"Doğrulanmamış AI çıktısı"` (TR) before the softer explanation, aligning with `PolicyCard.tsx` + the toast wording. `TrustworthinessUI.test.tsx` asserts `/UNVERIFIED AI OUTPUT/i` against the banner — when adding new unverified-state paths, ensure this label stays prominent.

79. **`vi.hoisted()` must wrap CLASS declarations for mock factories (Added Apr 18, 2026)**: The general `vi.hoisted()` pattern (gotcha referenced at the vi.hoisted entry above) is well-known for `vi.fn()` spies, but a subtler variant bites when the test needs to return a CLASS from `vi.mock()` (e.g. an Error subclass used for `instanceof` checks in the component under test). A top-level `class MockErr extends Error {}` referenced from inside `vi.mock()` throws `ReferenceError: Cannot access 'MockErr' before initialization` because `vi.mock()` is hoisted above all top-level statements — including class declarations. **Fix**: move the class INSIDE `vi.hoisted()`: `const mocks = vi.hoisted(() => { class MockErr extends Error {...}; return { MockErr, fetchFoo: vi.fn() } })`. Working example in `src/components/admin/tabs/SegmentsTab.test.tsx`.

80. **Email resolver caps at 1000 auth.users (Added Apr 18, 2026)**: `POST /api/admin/app-users/resolve-emails` paginates via a single `supabase.auth.admin.listUsers({ perPage: 1000, page: 1 })` call. Platforms with more than 1000 users will see emails falsely reported as "missing" when they actually exist beyond page 1. The response includes `cappedAtUserListLimit: true` so the UI can surface a warning. If the platform scales past 1000 users, upgrade the endpoint to a server-side SQL function that queries `auth.users` by email index (requires service-role client — the `auth` schema isn't reachable via standard supabase-js `.from()`).

81. **`evaluateKaskoPilotGate()` anonymous bypass (Added Apr 18, 2026)**: When `userId` is `undefined`, the gate skips BOTH the rollout-bucket check (can't hash without userId) AND the segment membership check (anonymous → `userInSegment = true` via `|| !userId`). This is intentional — anonymous TryAnalysis uploads must keep working — but it means setting `rolloutPercentage = 0` does NOT block anonymous users; it only blocks logged-in non-segment users. If Phase F needs to lock anonymous paths to the same rollout percentage, add a separate rollout percentage for anonymous users or treat anonymous as bucket 0 (always-in).

82. **Migration 040 file-vs-prod divergence (Added Apr 18, 2026)**: The migration file was edited Apr 18 to rename both RLS policies to `"Open read/write (API layer enforces auth)"` + add inline intent comments. **The migration already ran in prod**, so the live DB still carries the original policy names (`"Service role manages segments"` and `"Service role manages QA records"`). The policy bodies are identical; only names differ. Fresh installs / local dev get the corrected names. Do NOT write a rename migration unless there's operational need.

83. **Read-only-by-default for operator repair scripts (Added Apr 18, 2026)**: Any one-shot script under `scripts/` that mutates the `policies` (or similar) table in prod MUST default to a read-only audit mode and require an explicit `--apply` flag for writes. The old `backfill-date-bug.ts` wrote immediately on invocation — an operator couldn't diagnose scope without mutating. The new contract: `--audit-only` (default) emits counts + CSV, `--apply` runs writes behind an interactive `promptConfirm()` (skippable via `--yes` for CI). Audit and apply modes MUST share a single classifier function (e.g. `classifyRow()`) so the report reflects exactly what `--apply` would touch. Also: `--apply` should write an `audit-applied-<timestamp>.csv` log with the rows updated, for rollback reference. See `scripts/backfill-date-bug.ts` as the canonical template and `docs/runbooks/05-date-corruption-audit.md` §4.4 for the operator workflow. **Parser-drift trap specific to this script**: the `--apply` write path calls `parseExtractedDate` from `scripts/_simple-date-parser.ts` (the pilot-script mirror), NOT production's `parseTurkishDate()` in `turkish-utils.ts`. Per gotcha #16, the pilot scripts can't import from `src/lib/` (Vite `import.meta.env` crash). Per non-negotiable rule #11, the two parsers must stay in sync manually. **Before any prod `--apply` run**, run the 22-test drift canary `npx vitest run scripts/__tests__/simple-date-parser.test.ts` — if it fails against current production behavior, the mirror is stale and `--apply` would silently re-introduce corruption.
...
# Admin Email Resolver — OPT-IN, privacy-sensitive (Added Apr 18, 2026)
# When true, POST /api/admin/app-users/resolve-emails resolves emails → UUIDs
# via auth.admin.listUsers. Used by Segments admin tab's "Paste Emails" mode.
# Default: disabled (endpoint returns 403 RESOLVER_DISABLED). See ADR-0004.
