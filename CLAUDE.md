...
4. **🔴 Pilot Batch Ingestion — Phase A script ready, Phase B + C pending (as of Apr 11)**:
   - ✅ **Phase A DONE** (commit `e1174df`): `scripts/pilot-batch-ingest.ts` extended with Document AI OCR fallback, `policies` table writer, duplicate guard, `--persist-policies` + `--reviewer-id=<uuid>` flags, preflight validation, and skip-label bugfix. Code-only — no credentials, no DB writes. Dry-run verified clean against 10 PDFs in `upload/real-kasko-pdf/`.
   - ✅ **Phase A+1 DONE** (commit `3cd6445`): preflight validation for `--persist-policies` + handoff docs sync.
   - ✅ **Phase A+2 DONE** (follow-up commit): runbook `docs/runbooks/03-pilot-batch-ingestion.md`, extracted `parseExtractedDate` into `scripts/_simple-date-parser.ts`, 22 unit tests in `scripts/__tests__/simple-date-parser.test.ts`, production bug flagged (see #4b).
   - 🔴 **4b. NEW — Latent production bug in `src/lib/ai/policy-extractor.ts:1609-1637`**: V8's `Date` constructor silently mis-parses Turkish `DD.MM.YYYY` strings where day ≤ 12 as MM.DD.YYYY (day/month swap). Affects roughly half of all real KASKO policies. Pilot batch script has the fix via `_simple-date-parser.ts` — **production still has the bug**. Needs: ~5-line fix in `policy-extractor.ts`, regression tests, decide whether to backfill existing `policies` rows. Full diagnosis + reproduction + fix recipe in SESSION_HANDOFF.md.
   - 🔴 **Phase B (user action, pending)**: Create `/home/user/insurai/.env` with `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `GCP_SERVICE_ACCOUNT_BASE64`, and `PILOT_REVIEWER_USER_ID` (real `auth.users` UUID — `policies.user_id` has NOT NULL FK). `.env` is in `.gitignore`. See `docs/runbooks/03-pilot-batch-ingestion.md` §1 for the full list + 3 ways to source a reviewer UUID.
   - 🔴 **Phase C (after Phase B)**: Run `npx tsx scripts/pilot-batch-ingest.ts ./upload/real-kasko-pdf --persist-policies`. Expected ~3-5 min wall time, ~$0.20-0.50 in API tokens, 8-10 new rows in `policies` table. Preflight fails fast if env is missing — no tokens wasted on broken runs.
...
52. **🔴 V8 `Date` constructor silently mis-parses Turkish `DD.MM.YYYY` when day ≤ 12 (Added Apr 11, 2026)**: `new Date('01.12.2024').toISOString()` returns `'2024-01-12T00:00:00.000Z'` — January 12, NOT December 1. Node only rejects (NaN) when day ≥ 13 (e.g. `new Date('15.12.2024')` → Invalid). This means any code that does `let d = new Date(turkishDateString); if (isNaN(d.getTime())) { /* manual parse */ }` silently produces day/month-swapped dates for roughly half of all real Turkish KASKO policies. **Latent in production at `src/lib/ai/policy-extractor.ts:1609-1637`** (affects `start_date`, `expiry_date`, and derived `status`). **Fixed** in `scripts/_simple-date-parser.ts` (pilot batch only) by running the manual 3-part split parser FIRST before falling back to `new Date()`. See SESSION_HANDOFF.md priority #4b for the production port + regression test + backfill decision. When writing any Turkish date parsing: always run the manual `DD.MM.YYYY` regex before `new Date()`. Audit query for existing corrupted rows: `SELECT id, start_date, expiry_date, raw_data->>'startDate' FROM policies WHERE type = 'kasko' AND raw_data->>'startDate' ~ '^\\d{1,2}\\.\\d{1,2}\\.\\d{4}$';`.
...
- **Last Updated**: April 11, 2026 (Pilot batch Phase A → A+2 complete: `scripts/pilot-batch-ingest.ts` extended with Document AI OCR fallback + `policies` table writer + preflight validation, extracted `scripts/_simple-date-parser.ts` with 22 unit tests, runbook `docs/runbooks/03-pilot-batch-ingestion.md`. Commits `e1174df`, `3cd6445`, `ea97912`. **Latent V8 `Date` DD.MM.YYYY bug discovered** in `policy-extractor.ts:1609-1637` — pilot fixed, production pending as task #4b. Phase B/C still blocked on env delivery.)
...
11. `scripts/_simple-date-parser.ts` `parseExtractedDate()` intentionally diverges from production `policy-extractor.ts:1609-1637` to fix the V8 DD.MM.YYYY day≤12 bug (gotcha #52). Keep the shared module as authoritative for pilot scripts until production is patched (task #4b). Do NOT "sync" it back to match broken production behavior.
