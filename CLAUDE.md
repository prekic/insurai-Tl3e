...
2. **Optional follow-up PRs** for the two items deferred from #331/#332 (see `SESSION_HANDOFF.md` for context):
   - **Schema unification**: extract client + server `extraction-schema.ts` into a single `shared/extraction-schema.ts` source. Audit findings from Apr 8 session showed 5 categories of subtle drift remaining post-fix, including a CONTRADICTION in the currency description.
   - **Strict-mode CI validator**: extract `validateStrictCompliance()` from `server/schemas/extraction-schema.test.ts:189-228` into a reusable utility, wire into both schema test files as a deterministic CI gate (replaces the manual OpenAI API reproduction step).
...
48. **`as unknown as` casts hide structural bugs (Added Apr 8, 2026)**: The L-3 safe-default in `engine.ts` was deliberately cast through `unknown` to bypass TypeScript (`{ overall: 0, components: {} } as unknown as AnalysisBundle`). This pattern silently broke the contract between `generateAnalysisBundle` and its consumers and crashed `evaluateDisplayMode` in `review-thresholds.ts:182` when the safe-default actually fired ("Cannot read properties of undefined (reading 'extractionQualityScore')"). When you see `as unknown as X` in production code, treat it as a code smell and verify the shape really matches the type — if it doesn't, refactor instead of casting. The `createSafeDefaultBundle()` helper at `src/lib/analysis/engine.ts` (commit `1cf9a96`) is the canonical pattern: any function that returns a complex type with multiple consumers should construct its safe-default fallback as a complete instance via a helper, not as a partial inline stub. The complete-stub pattern restores end-to-end type safety AND eliminates the need for optional chaining in every consumer.

49. **Server schema strict-mode parity is enforced; client+server must mirror until unified (Added Apr 8, 2026)**: `server/schemas/extraction-schema.ts` is documented as strict-mode compliant in its file header. ALL properties (including new additions) must be in `required[]`. The recursive `validateStrictCompliance()` test at `server/schemas/extraction-schema.test.ts:189-228` enforces this on every test run. When adding new top-level fields, update both `properties` AND `required[]` AND ensure every nested object's own `properties`/`required[]` are in lockstep. The client schema (`src/lib/ai/extraction-schema.ts`) follows the same pattern after commit `28827fd` — both are now consistent at 19 top-level required fields each (`policyNumber`, `provider`, `policyType`, `insuredName`, `insuredAddress`, `startDate`, `endDate`, `premium`, `currency`, `paymentFrequency`, `coverages`, `specialConditions`, `exclusions`, `exclusionsEn`, `conditionalDeductibles`, `evidence`, `clauseGraph`, `amendmentInfo`, `confidence`). **Known follow-up**: extracting both into a shared `shared/extraction-schema.ts` source is deferred to a future PR; until then, any change to either schema MUST be mirrored to the other manually. Audit findings from Apr 8 also revealed 5 categories of subtle drift (different nullable patterns, missing `coverages[].nameTr` on server, contradicting `currency` description, missing `EXTRACTION_SYSTEM_PROMPT`/type exports on server) that the unification PR should resolve.
...
- **Last Updated**: April 8, 2026 (3 latent bug fixes #331/#332/#333 + QA audit correction: full branch scope documented in SESSION_HANDOFF.md, gotchas #50/#51 added for `onStageChange` pub/sub and `translations-skeleton.ts` rule nuance)
