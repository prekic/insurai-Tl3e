...
128. **Stream-Based API for Massive Policies (Added April 26, 2026)**: To prevent truncation on massive KASKO policies (46k+ characters) when using SOTA models (like Sonnet 4.6), the extraction pipeline relies on stream-based API usage with a 32,768 token limit. This ensures the output JSON is not abruptly cut off.
