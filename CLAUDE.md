...
1. **Pilot Batch Ingestion**: Bulk upload the remaining `upload/real-kasko-pdf/` files into the web UI application.
2. **Execute Evaluation Backfill**: Run `npx tsx scripts/backfill-evaluation-scores.ts --apply` to generate database scores for all ingested policies.
3. **Calibrate Grade Thresholds**: Run `scripts/calibrate-grade-thresholds.ts` once 50+ scored policies exist to adjust the A/B/C/D grade cutoffs.
4. **Update Benchmark Data** *(blocked — needs market research)*: Premium ranges still from Dec 2024. Needs external research to update `MARKET_BENCHMARKS`.
...
- **Last Updated**: April 4, 2026 (Trustworthiness UI polish, AI insight wording softening, history chart fix, unverified score visual gating)
