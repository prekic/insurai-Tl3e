...
1. **Open PR for v4 work**: Branch `claude/load-project-context-bNYCu` is 3 commits ahead of `main` with the v4 extraction-depth pass (alias table + completeness gate, tightened PDF golden assertions, Ek Sözleşme bullet parser + named deductibles + IMM carve-out caveat). Open a PR and merge once reviewed. Suggested title: `feat(extraction): v4 extraction-depth pass — alias table, completeness gate, Ek Sözleşme bullets, deductible enumeration, IMM carve-out`.
2. **Allianz inverted-label make extraction**: Tracked latent limitation flagged in PR-2. The Allianz Peugeot format `: PEUGEOT (114)\tMarka` puts the make VALUE before the LABEL on the same line. `extractVehicleInfoFromText` returns `make: undefined` for this case. Fix requires a bidirectional scan in `matchLabeledField`. Currently routed via `extractorLenientFor: ['make']` in `qa-pdf-golden.test.ts`.
...
5. **Pre-existing Test Failure**: `src/lib/ai/pdf-parser.test.ts` has a length threshold assertion mismatch — unrelated to evaluator/extraction changes. Fix before next major PR.
