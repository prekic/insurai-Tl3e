...
1. **Self-audit layer (Phases 1-4) is shipped and code-complete.** Migrations 053 (`audit_judgements`), 054 (`Audit Judge - Kasko` prompt + 3 `app_settings` rows), and 055 (`audit_trend_snapshots`) are applied to production Supabase. Live policy uploads now fire-and-forget a per-typology audit-judge run; weekly trend tracking runs via `.github/workflows/audit-judge-trends.yml` (manual `workflow_dispatch` only — `schedule:` cron is commented out until 1-2 clean manual runs confirm calibration).
2. **Workflow run #4+ should be green** after the May-1 noise-floor fix. The trend tracker now requires `baseline >= 5` of a metric before drops can flag CRIT (was 3). Anthropic re-bucketing 3 exclusions into adjacent fields no longer trips the gate. See gotchas #142-#147 below.
3. **Apply Migration 049** (named-deductible attribution rules) to production Supabase if not already done — it shipped Sprint 1 but the Apr-30 audit showed `cd=0` across all 13 corpus PDFs, suggesting either the migration didn't apply OR the corpus genuinely lacks named-percentage scenarios. Verify both possibilities. The reviewer's exact Anadolu Birleşik Kasko fixture (with %35/%80 scenarios) is NOT in the corpus — manually upload it to confirm migration 049 is firing.
...
10. **Three deferred audit-layer follow-ups** (post-ship): (a) `cost_usd` is null on every `audit_judgements` row because `calculateCost()` lacks `claude-sonnet-4-6` pricing — non-blocking but blocks daily-budget circuit breaker accuracy; (b) admin notifications aren't firing on critical findings despite `critical_count=3` on persisted rows — investigate `notifyAuditQuality()` wiring; (c) `normaliseInsurer()` in `src/lib/audit/typology.ts` doesn't NFKC-normalise or collapse internal whitespace, so two slightly-different insurer strings can hash to different typologies and bypass the cache.
...
- **Last Updated**: May 1, 2026 (Self-audit layer Phases 1-4 shipped: deterministic detectors (`src/lib/audit/quality-detectors.ts`), render-contract Playwright tests (`e2e/render-contract.spec.ts`), LLM-as-judge layer with typology cache + daily-budget breaker + quote-verification (`server/services/audit-judge-service.ts` + `src/lib/audit/judge-client.ts`), Phase 4 fixture-level trend tracking (`scripts/audit-trend-track.ts`, `audit_trend_snapshots` table), per-policy fire-and-forget judge hook in `policy-extractor.ts`. Migrations 053/054/055 applied to production. Workflow `.github/workflows/audit-judge-trends.yml` is workflow_dispatch-only until calibrated. May-1 noise-floor fix: `MIN_BASELINE_FOR_REGRESSION` 3 → 5 after Run #3 tripped CRIT on Anthropic exclusion re-bucketing. Three audit-layer follow-ups deferred (cost_usd null, admin notifications not firing, normaliseInsurer NFKC gap — gotchas #143-145).)
