...
1. **Merge Handoff Branch to Main**: Branch `claude/project-handoff-docs-0XYw6` has 8 commits ahead of `main` (isDraft persistence, admin benchmark config, test/TS fixes, PWA icons, ComparePolicies tests, docs). Create PR and merge.
2. **Deploy to Production**: Railway deployment needed — sandbox `git push` doesn't trigger Railway webhook. Use `mcp__github__push_files` to create a real GitHub commit event, or trigger Railway manual deploy.
3. **Apply Migrations to Production Supabase**: `042_add_is_draft_to_policies.sql` (isDraft persistence) and `043_seed_benchmark_threshold_configs.sql` (admin-configurable freshness thresholds).
...
- Standard OCR processor (`c2741b178ab61433`) has **15-page API limit** per request, but `DOCUMENT_AI_PAGE_LIMIT` is set to **10** in `pdf-splitter.ts` to avoid 403 payload errors on dense PDFs
- `enableImagelessMode` is **NOT supported** on standard OCR processors (Enterprise only)
- PDFs >10 pages are automatically split into chunks via `pdf-splitter.ts`
- Check logs for version marker: `[Document AI] OCR route v5 invoked (standard processor)`
- GCP credentials can be passed via `GCP_SERVICE_ACCOUNT_BASE64` (base64-encoded JSON)
- For documents >10 pages, chunks are processed separately and results combined
...
**i18n Ternary Migration — S1+S2 Complete, FX Remaining (Updated Mar 4, 2026):**
- Plan location: `/root/.claude/plans/warm-tickling-wilkes.md` — 10 steps covering S1/S2/S3/FX
- **Plan execution: ~75%** — P0 (test fixes), P1 (locale-aware formatting), P2 (S1 components), P3 (S2 components) all DONE
- **99 inline `locale === 'tr'` ternaries replaced** with translation keys across 8 components
- **4 data-field ternaries remain** (correct — these select between pre-existing `labelTr`/`label` on data objects, not translation strings):
  - `PolicyDiffViewer.tsx:50` — `change.fieldLabelTr` vs `change.fieldLabel`
  - `PolicyCard.tsx:49` — `policyTypeInfo?.labelTr` vs `policyTypeInfo?.label`
  - `AIInsightsPanel.tsx` — `insight.textTr` vs `insight.text`
  - `EmailPreferences.tsx:160-161` — `config.labelTr`/`config.descriptionTr` vs `config.label`/`config.description`
- **Translation sections added**: `conflictResolution` (50 keys), `aiInsights` (32 keys), `policyDiff` (15 keys), `policyDocuments` (12 keys), `emailPreferences` (6 keys), `errorBoundary` (7 keys), `notFound` (4 keys), `policyCard` (8 keys) — all in EN/TR/skeleton
- **S1 components fully migrated**: `NotFound.tsx`, `ErrorBoundary.tsx` (wrapper pattern), `PolicyDocuments.tsx`, `AIInsightsPanel.tsx`
- **S2 components fully migrated**: `ConflictResolutionDialog.tsx`, `PolicyDiffViewer.tsx`, `PolicyCard.tsx`, `EmailPreferences.tsx`
- `PolicyDetailView.tsx` has **124 additional ternaries** (not in plan scope but largest in codebase)
- **Remaining**: FX conversion system (steps 2, 6-10) and PolicyDetailView ternaries
- See `SESSION_HANDOFF.md` for next steps
...
**ErrorBoundary i18n Wrapper Pattern (Added Mar 4, 2026):**
- `ErrorBoundary.tsx` is a class component that cannot use hooks directly
- Solution: `ErrorBoundaryWrapper` function component wraps the class, providing `t` and `locale` via `useI18n()` and passing them as props
- The class component receives translations through `this.props.translations` and `this.props.locale`
- `ui/error-boundary.tsx` follows the same pattern
- When creating new class components that need i18n, use this wrapper pattern rather than trying to call hooks inside class methods

**i18n Ternary Migration — Data-Field vs Translation Ternaries (Added Mar 4, 2026):**
- After the S1+S2 migration, 4 `locale === 'tr'` ternaries remain across the migrated components — all are **data-field selections** that are correct to keep
- Data-field pattern (KEEP): `locale === 'tr' ? item.nameTr : item.name` — selects between fields that already exist on the data object
- Translation pattern (REPLACE): `locale === 'tr' ? 'Türkçe string' : 'English string'` — hardcoded strings that belong in translation dictionaries
- When auditing remaining ternaries (e.g., PolicyDetailView's 124), apply this distinction — only migrate hardcoded string ternaries, leave data-field selections in place
- New translation sections added to `translations.ts` interface: `conflictResolution`, `aiInsights`, `policyDiff`, `policyDocuments`, `emailPreferences`, `errorBoundary`, `notFound`, `policyCard`

**i18n Test Mock — Must Match Import Path (Added Mar 4, 2026):**
- All 8 S1+S2 migrated components import `useI18n` from `@/lib/i18n` (barrel export). Tests MUST mock `@/lib/i18n`, NOT `@/lib/i18n/i18n-context`
- Mocking `@/lib/i18n/i18n-context` only intercepts components that import the context module directly (some older tests use this)
- This session's tests use **inline translation objects** in the mock (only provide the sections the component uses), NOT a full `EN_TRANSLATIONS` import
...
**useDisplayCurrency Test Mock Pattern (Added Mar 6, 2026):**
- All components that display currency values now import `useDisplayCurrency` from `@/hooks/useDisplayCurrency`
- Tests for these components MUST mock the hook: `vi.mock('@/hooks/useDisplayCurrency', () => ({ useDisplayCurrency: () => ({ displayCurrency: 'TRY', convert: (v: number) => v, formatConverted: (v: number) => '₺' + v, formatConvertedCompact: (v: number) => '₺' + v.toLocaleString(), isReady: true }) }))`
- Without this mock, tests will fail because `useDisplayCurrency` depends on `FXService` which needs a server endpoint
- The mock returns identity conversion (`convert: v => v`) and simple string formatting — sufficient for assertions
- 7 test files already have this mock: PolicyCard, PolicyDashboard, PolicyDashboard-branches, PolicyDetailView, PolicyDetailView-branches, PolicyDiffViewer, medium-coverage-branches
- When adding new components that display currency, add the `useDisplayCurrency` call and update the corresponding test file with the mock

**formatConverted in useCallback Dependencies (Gotcha Mar 6, 2026):**
- `formatConverted` from `useDisplayCurrency` is a function reference that changes when `displayCurrency` changes
- It MUST be included in `useCallback`/`useMemo` dependency arrays when used inside the callback
