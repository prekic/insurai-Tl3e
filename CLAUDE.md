...
1. **Deploy & Verify AI Insights Management**: This session added a new Supabase table (`ai_insight_guidelines`) and an Admin UI (`InsightsTab.tsx`) to dynamically manage AI sense-check rules to prevent AI hallucination. Deploy this branch, then log in as an administrator, go to `/admin/insights`, and create a test rule (e.g., policy type `*`, region `*`, guidance "Test rule") and observe the extraction output.
2. **Deploy & Verify Bilingual Extraction**: Previous commits added `quoteTr` to the `extraction-schema.ts` and Anthropic prompts for bilingual evidence display.
...
- **Last Updated**: March 14, 2026 (Anonymous Extraction Persistence, AI Premium vs Vehicle Value magnitude checks, Dynamic AI Insight Guidelines, proxy timeout handling, VKN/TC Kimlik fixes, Editable AI Prompts, Pipeline Execution Schema, Duplicate Insights Fix, PDF Extraction ArrayBuffer fixes, jsdom Worker Fix, admin password reset procedure)
