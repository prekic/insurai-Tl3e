...
- **Last Updated**: April 26, 2026 (v5 extraction-depth pass: integrated Gemini 2.5 Flash via `@google/genai`; hardened normalizers with `typeof` guards; removed artificial free-trial bottlenecks; added mandatory IMM/sigortaBedeli rules; upgraded exclusion dedup to trigram Jaccard; split premium Net/Tax; suppressed unverified policy scores; fixed 10x premium inflation bug; updated schema strict mode; enforced `npm run qa:extraction` backend gate before UI deployment; implemented SSE keepalive for Railway 30s timeout; added Gemini OCR fallback to client chain; fixed processing log race conditions; added real PDF pipeline integration test; updated `SESSION_HANDOFF.md`.)
