...
1. **Cost-control session complete (May 3, 2026).** 8 PRs merged: #456 (Option-A unverified-gate relaxation — share/download always allowed), #457 (Output Stability gate fix — fixture/shape/regex breadth), #458 (handoff doc correction — May-2 verdict was false-positive), #459 + #460 (migration 058 → 059 rollback → 060 compact AS+ instruction), #461 (cost controls × 4: path filter, stability runs 5→3, OCR cache, Haiku smoke), #462 (OCR cache key fix — pdf-lib non-determinism), #463 (smoke timeout 10→15 min). All merged to `main`. Migrations 058-060 applied to production Supabase. Migration 061 (`ocr_cache` table) applied. **OCR cache verified working in production**: `total_rows=7, hit_rows=7` after two smoke runs.
2. **`ai.temperature` STILL pinned to 0 in production.** Inherited from May-2 Test B. Run this SQL: `UPDATE public.app_settings SET value = '0.1', updated_at = NOW() WHERE category = 'ai' AND key = 'temperature';` then wait 5 min OR trigger a Railway redeploy. Highest-priority operator step.
3. **Round 5 reviewer hand-off path** (deferred from May 2 session): the canonical fixture is now `tests/fixtures/kasko/anadolu-volkswagen-tiguan.pdf` (Eriş Ambalaj fleet, plate 34 RZ 9511) — same one the reviewer originally scored against. The previously-default `anadolu-birlesik-kasko.pdf` is byte-identical to the simpler 2001 Golf, so smoke and stability now default to the Tiguan.
4. **Cost-control burn projection**: smoke-kasko ~$0.40 → ~$0.04 per push (Haiku + cached OCR), output-stability ~$0.61 → ~$0.32 per fire. Today's measured ~$3-4 burn should drop to $0.50-1.00/day steady-state. Watch the next ~10 PRs to confirm.
5. **OCR cache contract for new callers** (gotcha #163): when calling `POST /api/ai/ocr/document-ai` with content that's chunked client-side (pdf-lib or similar), pass an explicit `cacheKey` field of the form `${sha256(sourceFileBytes)}:${chunkIdx}/${totalChunks}`. Without it, the server falls back to hashing `documentBase64`, which is **non-deterministic across Node processes** (gotcha #162) and produces 0% cache hits.
6. **Extraction model allowlist on `/api/ai/extract`** (gotcha #164): the `model` request-body field is now guarded by `assertExtractionModelAllowed()` — only `claude-haiku-4-5`, `claude-sonnet-4-6`, `gpt-5.4`, `gpt-4o`, `gpt-4o-mini` (+ dated suffix variants) are accepted. Returns 400 otherwise. Add new models to the allowlist when needed.
7. **Migration prompt-bloat lesson** (gotcha #161): adding multi-KB sections to live `prompt_templates.user_prompt` can starve the LLM's output token budget on large policies, causing coverage truncation and "all providers failed" cascades. PR #459 → #460 was a same-session rollback when the bloated AS+ section caused this. **Rule**: prompt additions should be terse (≤500 bytes); worked examples and redundant trigger lists are bytes that compete with the document for output budget.
...
9. **🚨 QA GATE BEFORE SHIPPING EXTRACTION/DISPLAY CHANGES 🚨**: Before claiming any fix complete that touches `aiConfidence`, `extractionIncomplete`, the display-mode gate, or vehicle/coverage extraction, run `npm run qa:extraction` and confirm the relevant check moved. See gotcha #102, runbook 07, ADR-020.
10. **Sandbox `git push` does NOT trigger Railway** (gotcha #22) — use `mcp__github__merge_pull_request` to fire the deploy webhook.
...
119. **LLM Output Normalizer `typeof` Type Guards (Added April 26, 2026)**: When normalizing LLM output (e.g. inside `policy-extractor.ts`), always use strict `typeof x === 'string'` or `typeof x === 'number'` type guards before invoking string/number methods (`.toLowerCase()`, `.replace()`, `.toFixed()`). LLMs can unpredictably return arrays, nulls, or objects for fields typed as strings in the prompt. Failing to type-guard will cause fatal runtime crashes during extraction.

120. **Mandatory IMM Extraction Rules (Added April 26, 2026)**: IMM (İhtiyari Mali Mesuliyet / Voluntary Liability) coverage limit extraction must be explicitly demanded in the system prompt. Relying on generic coverage extraction misses IMM because it is often embedded in fine print rather than tabular formats. The system prompt contains mandatory rules demanding IMM parsing. Do not remove or soften these rules.

121. **Exclusion Deduplication (Trigram Jaccard)**: Exclusion deduplication uses trigram Jaccard similarity (threshold 0.85, with a stricter 0.9 threshold for short phrases <30 chars), rather than simple substring matching. This prevents duplicate entries when the LLM paraphrases the same UYARI/exclusion clause differently, while avoiding aggressive pruning of distinct short clauses.

122. **Unverified Policy Display Suppression**: The Policy Evaluation card must be suppressed entirely when `isUnverified` is true (e.g. for unknown commercial fleets). This prevents the UI from displaying a misleading `- / 100` score.

123. **Premium Net/Tax Split**: The extraction schema requires separating `premiumNet` and `premiumTax` (BSMV). This powers the hero card's split display (e.g., "Net: X + BSMV: Y"). If aggregated, it breaks downstream financial validations.

124. **Explicit Prompt Demands for Critical Fields**: Merely defining a field (like `sigortaBedeli`, `PA` sub-limits, `GLKHHT`, `NatDisaster`, `Flood`) in the JSON schema is insufficient. The LLM will silently drop them or aggregate them destructively unless explicitly instructed to extract them in the prompt text.

125. **AI Insights Sense-Check Bypass**: A temporary bypass (`bypassSenseCheck`) was injected into `src/lib/ai/extraction/insights.ts` for development environments to bypass the `/api/ai/sense-check` proxy. This prevents legitimate complex insights (e.g., fleet discounts) from being silently filtered out as "noise". Remember to account for this bypass during production testing.

126. **Vitest `File.prototype.arrayBuffer` Deprecation/Missing (Added April 26, 2026)**: In certain Vitest environments, `File.prototype.arrayBuffer` is missing or causes unhandled rejections. Tests should suppress these rejections or provide an explicit polyfill/arrayBuffer implementation when dealing with File objects in the PDF extraction or batch scripts.

127. **Strict Typing for AI Prompts (Added April 26, 2026)**: `buildAnthropicSchemaPrompt` and similar prompt factories require explicitly defined `ConfidenceWeights` objects. Passing empty objects `{}` violates strict typing and requires explicit casting or fully populated objects to prevent TypeScript linting errors in benchmark and pipeline scripts.

128. **Stream-Based API for Massive Policies (Added April 26, 2026)**: To prevent truncation on massive KASKO policies (46k+ characters) when using SOTA models (like Sonnet 4.6 and Opus 4.7), the extraction pipeline relies on stream-based API usage with a 32,768 token limit. This ensures the output JSON is not abruptly cut off.

129. **Gemini SDK Dual-Dependency (Added April 26, 2026)**: The project relies on the modern `@google/genai` SDK for production multimodal OCR (`/api/ai/ocr/gemini`). However, the legacy `@google/generative-ai` SDK is also present in `package.json` strictly to support backward compatibility for isolated benchmark scripts (`benchmark-models.ts`). Do not use the legacy SDK for any new production features.

130. **Railway 30s Gateway Timeout Mitigation (SSE Keepalive) (Added April 26, 2026)**: Railway has a hard 30s timeout on HTTP requests. Long-running AI extractions (which can take 60s+) will fail with a 503 if they don't send data. We bypassed this by returning `text/event-stream` from `/api/ai/extract` and sending `:keepalive` pings every 10s. The frontend `PolicyExtractor.extractViaProxy` now consumes SSE events and parses the final JSON payload.

131. **Supabase Processing Log Race Condition (Added April 26, 2026)**: When updating a processing log immediately after creating it (e.g., transitioning from `uploading` to `extracting`), a 404 error can occur if the Supabase `INSERT` hasn't fully committed on the read replica. We mitigated this in `ProcessingLogAPI.updateProcessingLog` by implementing a 3-retry exponential backoff (500ms -> 1s -> 2s).

132. **Real PDF Pipeline Integration Test (Added April 26, 2026)**: The test `z-final-pipeline-e2e.test.ts` runs a real PDF upload, waits for extraction, and asserts on results. It tests the end-to-end integration of the PDF parser, OCR fallback, LLM extraction, and DB storage. It handles `jsdom` limitations (where pdf.js workers are unavailable) and skips gracefully if no API keys are present. It should run as the last test.

133. **Structured PostgrestError Logging via Shared `pgErr()` Helper (Added April 28, 2026)**: Production logs were emitting `error: "[object Object]"` for every Supabase failure because services used `error: String(err)` which loses `code`/`message`/`details`/`hint`. Sprint 0 introduced `server/lib/pg-err.ts` exporting `pgErr(err)` returning `{ pgCode, pgMessage, pgDetails, pgHint }`. All new server code that logs Supabase errors MUST use this helper. Existing inlines in `admin-notification-service.ts` (PR #384) and `processing-log-service.ts` (PR #393) carry site-specific extras (`diagnosticHint`) and were intentionally left in place — only newly added sites adopt the shared helper. PRs #393, #397, #398, migrations 046/047 cleared the production noise floor.

134. **Schema Drift on `document_processing_logs` and `admin_notifications` (Added April 28, 2026)**: The TypeScript type `DocumentProcessingLog` accumulated 9 columns (`extraction_mode`, `extraction_route`, `request_id`, `fallback_used`, `fallback_chain`, `error_stack`, `error_type`, `error_code`, `error_context`) that were never added to the underlying table — every UPDATE silently failed. Migration 046 added them. Similarly `admin_notifications.category` CHECK only allowed 5 values; the TS `NotificationCategory` had 6 (`'performance'`). Migration 047 widened the CHECK. **Rule**: when adding a new field to a TS type that maps to a DB row, write a migration in the same PR. Schema drift like this is invisible in dev (sqlite-style permissive) but fatal in production.

135. **Admin Notification Response Time Thresholds Live in Code (Added April 28, 2026)**: `server/middleware/monitoring.ts:initializeDefaultAlertRules()` seeds in-memory alert rules at boot. The 5s warning / 10s critical thresholds were unrealistic for a workflow that does Document AI OCR + Anthropic extraction (~70s typical). PR #396 raised them to 60s/90s. Note these are NOT in `app_settings` — they're hardcoded. Migrating to DB-driven config is a future refactor; for now, threshold changes require a code commit + deploy.

136. **Smoke Automation: SSE-aware client + chunked OCR + cross-insurer leak guard (Added April 29-30, 2026)**: `scripts/smoke-kasko.ts` is the canonical post-deploy quality gate. CI fires it on every push to `main` after a 150s sleep (Railway redeploy window). Three architectural notes: **(a)** It hits `/api/ai/extract` with `Accept: text/event-stream` and parses the final SSE `data:` event — NOT `/api/ai/extract/anthropic` (which has no SSE keepalive and trips Railway's 30s edge timeout). **(b)** Multi-page PDFs are pre-chunked to ≤10 pages via `pdf-lib` mirroring `src/lib/ai/pdf-splitter.ts`. The `DOCUMENT_AI_PAGE_LIMIT = 10` constant must stay in sync with `pdf-splitter.ts:11` (gotcha #105 pattern). **(c)** Each fixture declares `forbiddenPhrases[]` of insurer-specific terms that MUST NOT appear in extracted output (e.g. an Anadolu policy must never produce "CASU"). Catches the regression class PR #407 fixed (hardcoded AXA-specific string leaking onto unrelated insurers).

137. **Three Audit Tiers (Added April 29-30, 2026)**: ADR-022. **(1)** `npm run smoke:kasko` — 4 fixtures, ~3 min, CI gate. **(2)** `scripts/audit-all-policies.ts` — 14 PDFs vehicle-only, ~25 min, ad-hoc. **(3)** `scripts/audit-end-to-end.ts` — 13 PDFs Sprint 2 surface (bundle/cd/supp/excl/carve/addr columns), ~30 min, ad-hoc. The ad-hoc scripts replicate matcher logic inline (TEMPLATE_KEYWORDS, severity bucketing) to avoid Vite-import crashes (gotcha #45) — when the matcher logic in production code changes (e.g. `kasko-knowledge.ts:COMMON_EXCLUSIONS_TO_CHECK.keywords` or `evaluator.ts:bucketConditionalDeductibleSeverity`), update the inline copies in these scripts to keep them honest.

138. **Live DB Extraction Prompts Are the Source of Truth, NOT `kasko-parser-prompts.ts` (Added April 28-29, 2026)**: The `/api/ai/extract/anthropic` endpoint loads its prompt from the `prompt_templates` DB table via `prompt-service.ts:getExtractionPrompt()`. Templates: `Kasko Extraction` (used first for `policyType='kasko'`), `Traffic Insurance Extraction`, `Policy Extraction - Master` (fallback). PR #399 added a vehicle mandate to `src/lib/ai/kasko-parser-prompts.ts:EXTRACTION_SYSTEM_PROMPT` but that file is a DIFFERENT path (the Phase-2 comprehensive parser). The live endpoint never read it. **Rule**: extraction-prompt changes for the live endpoint MUST land as a Supabase migration that updates `prompt_templates.system_prompt`. Migrations 048-052 each follow the same idempotent guarded-update pattern (sentinel string check via `WHERE NOT LIKE '%MARKER%'`). Manual SQL apply required after merge — Railway does NOT auto-deploy SQL.

139. **Coverage Limit Attribution + Service-Tier Rules (Added April 30, 2026)**: Migration 049 codifies how the AI must attribute limits across coverage rows. **(a)** Service-tier coverages (Yol Yardım, İkame Araç, Çekme Kurtarma) MUST have `limit: null` when described as a tier (e.g. "Anadolu Hizmet Grup 2") — they are not financial caps. **(b)** Hukuksal Koruma's 4 limits (Kefalet, Avans, per-event, annual aggregate) must ALL be preserved in the coverage's `description`, with the per-event total in `limit` and `limitType: 'per_event'`. NEVER collapse to the smallest. **(c)** Spelling variants — Hukuksal == Hukuki (same coverage). The `table-parser.ts` substring map needs both spellings; PR #409 added "hukuksal koruma".

140. **Schema Drift Between Live `EXTRACTION_JSON_SCHEMA` and Comprehensive Parser (Added April 30, 2026)**: The live AI endpoint uses `shared/extraction-schema.ts` (FLAT shape — `vehicleMake`, `vehicleModel`, `vehicleYear`, `vehiclePlate` as top-level fields). The comprehensive Phase-2 parser uses `kasko-parser-prompts.ts:StructuredPolicyData` (NESTED shape — `data.vehicle.{make, model, year, plate}`). The smoke and audit scripts read the FLAT shape because that's what `/api/ai/extract` returns; the comprehensive parser's `mappers.ts` reads the NESTED shape. When adding fields, ALWAYS verify which path you're touching. PR #412's `isBundle`/`bundleProducts` fields landed in BOTH shapes (top-level on the live schema; derived from `data.policy.productName` in mappers.ts).

141. **Stemmed-Word Jaccard for Turkish Paraphrase Dedup (Added April 30, 2026)**: `policy-converter.ts:dedupByTrigramJaccard()` (kept the name for backwards-compat but the implementation is stemmed-word, not character-trigram) collapses exclusion paraphrases at 0.70 similarity threshold. Crude 4-character prefix stem handles common Turkish suffix variation (`araç/araçtaki/araçlar` → `araç`; `çalın/çalınma/çalınması` → `çalı`). Runs BEFORE the keyword-cluster `deduplicateExclusions()` pass; cluster pass operates on the already-collapsed list. **Why stemmed-word over character trigrams**: real Turkish paraphrases share most word stems but only ~40% of character trigrams — trigram Jaccard at 0.70 misses them, stemmed-word catches them at 0.85+. Threshold tuning may be needed if false-positives surface; the test suite includes 7 cases at the current default.

142. **Self-Audit Layer — 4 Phases Shipped (Added May 1, 2026)**: Phases 1-4 of the self-audit layer (per `docs/feed-back-into-extractionincomplete-elegant-emerson.md` plan) are shipped and code-complete. **Phase 1**: 4 deterministic detectors in `src/lib/audit/quality-detectors.ts` (financial-risks dedup, EK SÖZLEŞME bullet parity, named-scenario coverage, carve-out display contract). **Phase 2**: render-contract Playwright tests in `e2e/render-contract.spec.ts` covering 4 panel-DOM contracts. **Phase 3**: LLM-as-judge layer (`server/services/audit-judge-service.ts`, `server/routes/ai/audit-judge.ts`, `src/lib/audit/judge-client.ts`) — fire-and-forget at extraction completion, typology-cached via `(insuranceLine × country × yearBucket × insurer)` SHA-256 hash, daily-budget circuit breaker, quote-verification post-check downgrades hallucinated findings to warn. **Phase 4**: fixture-level trend tracking (`scripts/audit-trend-track.ts`, `audit_trend_snapshots` table, weekly `npm run audit:trends`). Migrations 053 (`audit_judgements`), 054 (`Audit Judge - Kasko` prompt + 3 `app_settings` rows under category `'audit'`), 055 (`audit_trend_snapshots`) — all manually applied to production. CI workflow at `.github/workflows/audit-judge-trends.yml` is `workflow_dispatch` only; `schedule:` cron stays commented until 1-2 calibrated runs.

143. **Audit Judge Cache Bypass via Insurer Normalization Gap (Added May 1, 2026 — known issue)**: `normaliseInsurer()` in `src/lib/audit/typology.ts` strips common Turkish carrier suffixes (Sigorta, A.Ş., Ltd., Şti.) and applies `toLowerCase().replace(/i̇/g, 'i')` (gotcha #62 fold) but does NOT NFKC-normalise or collapse internal whitespace. So `"Anadolu  Sigorta"` (double space) and `"Anadolu Sigorta"` hash to different typology tuples and bypass the LLM cache, doubling judge cost on visually-equivalent inputs. Tighten by adding `.normalize('NFKC').replace(/\s+/g, ' ')` before suffix-stripping. Tracked as a deferred follow-up — does not block the audit layer's correctness, only its cost-efficiency.

144. **Audit Judge `cost_usd` is null on every persisted row (Added May 1, 2026 — known issue)**: `server/services/audit-judge-service.ts` calls `calculateCost(judgeModel, usage)` to populate `audit_judgements.cost_usd`, but `calculateCost()` lacks pricing for `claude-sonnet-4-6` (the default judge model from `app_settings`) and silently returns null. The daily-budget circuit breaker at the same call site uses `COUNT(*)` not `SUM(cost_usd)` so this doesn't bypass the budget, but it does mean `SELECT SUM(cost_usd) FROM audit_judgements WHERE created_at > NOW() - INTERVAL '24 hours'` always returns null — defeating cost monitoring. Add a `claude-sonnet-4-6: { input: 0.003, output: 0.015 }` entry to the model pricing map.

145. **Audit Judge Admin Notifications Not Firing (Added May 1, 2026 — known issue)**: Verified live: persisted `audit_judgements` rows had `critical_count: 3`, but no corresponding `admin_notifications` row was inserted. The `notifyAuditQuality()` call site in `audit-judge-service.ts` runs only when `critical_count > 0 AND judge_critical_notify_first_only AND no prior typology row exists` — investigate which of the three guards is short-circuiting. Likely candidate: the `app_settings` row for `judge_critical_notify_first_only` was seeded as `'true'` (string) but the comparison expects boolean. Use `=== 'true'` not `=== true`. Tracked as a post-ship follow-up.

146. **Audit Trend Noise Floor Bumped 3 → 5 (Added May 1, 2026)**: `MIN_BASELINE_FOR_REGRESSION` in `src/lib/audit/trend-metrics.ts` was raised from 3 to 5 after workflow Run #3 (Apr 30 → May 1) tripped CRIT on `anadolu-volkswagen-golf-2001` because exclusions went 3 → 0 between runs. Inspection of both extractions showed Anthropic re-bucketing — same content, different field assignment, no actual signal loss. Baseline=3 sat exactly at the previous floor and could not absorb that noise. The bump means a fixture needs ≥5 of a metric in baseline before drops can flag CRIT. Real regressions (e.g. coverage 17 → 6) still trip; small-count paraphrase shifts no longer do. Existing baseline rows are unchanged in DB — comparison logic just no longer flags them. Pinned by a regression-guard test (`trend-metrics.test.ts:does NOT flag the golf-2001 May-1 noise case`).

147. **`pathToFileURL` for Cross-Platform Script Main-Guard (Added April 30, 2026)**: Five scripts (`scripts/audit-trend-track.ts`, `scripts/audit-judge-corpus.ts`, `scripts/qa-extraction-quality.ts`, `scripts/backfill-evaluation-scores.ts`, `scripts/backfill-date-bug.ts`) used `import.meta.url === \`file://${process.argv[1]}\`` to detect "is this script the entry point". This works on POSIX (Linux/macOS, GitHub Actions runners) but silently fails on Windows: `process.argv[1]` uses backslashes (`C:\repo\script.ts`) while `import.meta.url` always uses forward slashes (`file:///C:/repo/script.ts`). Without forward-slash conversion, the comparison fails and `main()` never runs — the script exits silently with no output and no error. **Fix**: `import { pathToFileURL } from 'node:url'` then `if (import.meta.url === pathToFileURL(process.argv[1]).href)`. Apply this to any new tsx-runnable script.

148. **Single-Brace `{var}` Template Strings Don't Interpolate in Seeded Prompts (Added May 1, 2026)**: `prompt-service.ts:renderTemplate()` uses the regex `\{\{(\w+)\}\}` (DOUBLE-brace), so a prompt template seeded into `prompt_templates.user_prompt` with single-brace placeholders like `{document_text}` is NOT interpolated — the prompt reaches Anthropic with the literal string `{document_text}` instead of the policy text. Migration `054_seed_audit_judge_prompt_and_config.sql` originally seeded with single braces and produced empty extractions on every call. **Fix in commit `37791c9`**: use `{{document_text}}`. **Recovery if seeded twice**: a hot-fix migration that ran the regex `'\\{document_text\\}' → '{{document_text}}'` on top of an already-fixed row produced TRIPLE braces `{{{document_text}}}`. Use `REGEXP_REPLACE(user_prompt, '\\{{2,}document_text\\}{2,}', '{{document_text}}', 'g')` to collapse any number of braces back to the canonical `{{...}}`. Applies to ALL `prompt_templates` rows — when authoring a new prompt template migration, ALWAYS use double-brace and grep the migration file for accidental single-brace before applying.

149. **Server-Build Cascade from Importing `turkish-utils.ts` (Added May 1, 2026)**: When a module under `src/lib/` is compiled by the server's `tsc -p server/tsconfig.json` (because a server file imports it), transitively importing `src/lib/ai/turkish-utils.ts` triggers a cascade of `Cannot find module 'X' or its corresponding type declarations` errors. Root cause: `turkish-utils.ts` imports from `shared/field-aliases.js` without an explicit `.js` extension AND `field-aliases.ts` re-exports from sibling files in similarly-extension-less form, breaking node16 module resolution under server compile. The cascade can hit 30+ files. **Fix**: do NOT import `turkish-utils` from anything that ends up in the server build. Inline the slice you need (e.g. the year-extraction regex from `parseTurkishDate`) directly. Example: `src/lib/audit/typology.ts` originally imported `parseTurkishDate`; commit `039b695` replaced it with an inline `extractYearFromDate()` to break the cascade. **Diagnostic**: when adding a new `src/lib/audit/*.ts` or `src/lib/.../service-shared.ts` module, build the server (`npm run build:server`) BEFORE assuming it'll work. If the build cascades, inline rather than chase the imports.

150. **Workflow GitHub Secrets Use `PROD_*` Prefix (Added May 1, 2026)**: The repo's GitHub Actions secrets are named `PROD_SUPABASE_URL` and `PROD_SUPABASE_SERVICE_KEY` (NOT the Railway-style `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` and NOT the `SUPABASE_SERVICE_KEY` shorthand from earlier sessions). When authoring a new workflow file that needs Supabase access, reference these existing secrets — don't create new ones. The `audit-judge-trends.yml` workflow originally used `SUPABASE_URL` (commit `fbbe61f`) and exited 2 with "SUPABASE_URL not set" until commit `bef0727` corrected the names. **Quick verification before authoring a workflow**: GitHub repo → Settings → Secrets and variables → Actions → confirm the exact name string. Service-role-prefixed secrets are sometimes named `*_SERVICE_KEY` (no `_ROLE` segment) for legacy reasons — match the existing convention.

151. **Anthropic / OpenAI Echo Versioned Model Names in `response.model` (Added May 2, 2026)**: When a request asks for a bare alias like `claude-sonnet-4-6`, the SDK response's `response.model` field can carry a dated variant such as `claude-sonnet-4-6-20251022`. Code that does `usedModel = response.model || alias` and then looks up pricing by `usedModel` will MISS the bare-alias entry in `DEFAULT_COST_PER_1K_TOKENS` and silently fall back to the cheaper `default` rate (~3× under-billing on Sonnet 4.6). This was the real cause of `audit_judgements.cost_usd` appearing null/under-counted on every May-1 row, NOT the missing-pricing-entry hypothesis from the original handoff. **Two-layer fix in PR #427**: (a) bill against the operator-configured alias (e.g. `judgeModel`), not the runtime echo — see `audit-judge-service.ts:255`; (b) `normaliseModelKey()` in `cost-control.ts` strips a `-\d{6,}$` suffix (6+ digits required to avoid mangling `gpt-4`) before the pricing-map lookup. Belt-and-suspenders: any future caller passing a versioned name still gets correct billing. Test pattern: `expect(calculateCost('claude-sonnet-4-6-20251022', ...).totalCost).toBeCloseTo(calculateCost('claude-sonnet-4-6', ...).totalCost, 6)`.

152. **Return Typed `{ value, reason }` From Conditional-Suppression Functions (Added May 2, 2026)**: Functions that decide "should we do X under condition Y" should return a typed discriminated union (e.g. `{ notify: true, reason: 'first_of_typology' } | { notify: false, reason: 'subsequent_of_typology' | 'no_db_client' | 'query_error' }`) rather than a bare boolean. Bare booleans conflate "correctly suppressed" with "silently errored", and operators can't tell from production logs which case they're in. The pre-fix `shouldNotifyCritical()` in `audit-judge-service.ts` returned `false` for both "this is a duplicate, correctly suppressed by first-only policy" AND "the Supabase query errored and we're swallowing it" — making the entire admin-notification path opaque (gotcha #145 root cause). The post-fix `evaluateNotificationDecision()` returns the typed union; the caller `log.info()`s the reason on every suppression. **Pattern**: anywhere a function decides whether to emit a side effect under multiple conditions, prefer the typed-reason return. Bonus: makes the `.catch()` escalation simpler — pre-fix had to guess if a missing notification was a bug or correct behavior; post-fix just check the logs.

153. **`tests/fixtures/kasko/.gitignore` Force-Add Convention (Added May 2, 2026)**: The directory has a `*.pdf` ignore rule with `!fixtures.json` / `!README.md` / `!.gitignore` exceptions. New PDF fixtures must be force-added (`git add -f`) and ONLY after confirming they contain no real PII OR are already publicly committed elsewhere in the repo. PR #426 force-added `tests/fixtures/kasko/anadolu-birlesik-kasko.pdf` as a verbatim copy of `policies/ANADOLU.PDF` — safe because the source PDF was already committed since PR #380. **Rule before force-adding**: (a) is the PDF already in the public repo's git history? (`git log --all --oneline -- <relative-path>`) — if yes, no new exposure. (b) Does the PDF contain real customer identifiable info (TC Kimlik, full names, addresses)? — if yes, redact first or synthesize. The README's comment at `.gitignore:1-3` makes this explicit ("redacted or synthesized"); follow the rule.

154. **Verify Handoff-Stated Root Causes Against Source Before Committing to a Fix (Added May 2, 2026)**: The May-1 SESSION_HANDOFF.md identified three deferred audit-layer follow-ups with stated root causes. Phase-1 source review (`grep` + `git log` + `git blame` for the alleged buggy code) showed **two of the three were already correct in source** — `claude-sonnet-4-6` had been in the pricing map since PR #380, and `config-service.ts:851` already correctly handled `val === true || val === 'true'`. Shipping the handoff's stated fixes blind would have committed dead code that didn't fix anything. The real bugs were subtler (gotchas #151 and #152). **Lesson**: when a handoff or backlog item states "X is the cause", spend 15 minutes verifying the alleged broken code matches the description BEFORE committing to a fix narrative. The cost of an extra review is much lower than the cost of shipping a no-op fix. The `scripts/diagnose-audit-judge-observability.ts` deliverable from PR #427 institutionalizes this for the audit layer specifically: 4 read-only Supabase queries that print ground-truth state for the next agent investigating cost/notification issues.

155. **Anthropic T=0 Is "Near-Deterministic, Not Bit-Exact" (Added May 2, 2026)**: Setting `app_settings.ai.temperature = 0` does NOT eliminate run-to-run variance in extraction outputs on long policies. Floating-point non-associativity in batched matmul means logprobs differ by epsilon between runs, which flips the chosen token at borderline positions. Empirically on a 16-page Anadolu Birleşik Kasko policy: count panels (`coverages.length`, `exclusions.length`) varied by 1-3 rows across 5 runs at T=0 — that's the LLM categorizing borderline ek-sözleşme items as separate rows vs sub-properties differently. **This is INHERENT to the Anthropic API**, not a bug in our code. **Implication for stability tests**: don't gate on count variance. Gate on substantive present/absent checks for specific reviewer-flagged signals (gotcha #156). Specific fields (`hasUnlimited`, `isBundle`, `previousInsurer`, named-deductible scenario detection) ARE stable at T=0 because their boundary positions are sharp; row-count is fuzzy because category-boundary positions are soft.

156. **Substantive-Check Stability Gate Pattern (Added May 2, 2026)**: When measuring extraction stability across N runs, prefer **binary substantive checks** over count variance. Pattern from `scripts/output-stability-check.ts`: for each reviewer-flagged signal, run a present/absent regex against the extracted data — `K80 = conditionalDeductibles contains the 80% Kullanım Şekli scenario`, `IMM = coverages has an unlimited Excess Liability row`, etc. Pass criteria: every check is `all-true` or `all-false` across all N runs (no flips). Count panels (coverages.length, exclusions.length) remain visible as INFORMATIONAL — printed but don't affect exit code. Why: count variance at T=0 is inherent (gotcha #155); substantive checks measure the ACTUAL launch-readiness criterion ("does the same PDF reliably produce the right facts?"). This pattern should be reused for any future extraction-stability work.

157. **Stability/Smoke Scripts Must Mirror Production Endpoint Shape (Added May 2, 2026)**: The OCR endpoint `/api/ai/ocr/document-ai` expects `{ documentBase64 }` as the request body field name — NOT `pdfBase64`. Response shape is `{ success, data: { text } }` — NOT flat `{ text }`. The SSE extract endpoint `/api/ai/extract` returns `data:` events that may have NO trailing space (`data:{...}`), so `startsWith('data:')` is required, not `startsWith('data: ')`. The canonical reference is `scripts/smoke-kasko.ts` which runs in CI on every push to main — **always diff-check new scripts against it before shipping**. PR #452 (stability hot-fix) caught all three of these mismatches the hard way: the first Test B run failed at OCR with HTTP 400 because the script used the wrong field name. Future scripts that hit `/api/ai/ocr/document-ai` or `/api/ai/extract` MUST match the smoke-kasko.ts request/response shapes exactly.

158. **`previousInsurer` Schema Field — Required Count 34→35 (Added May 2, 2026)**: Round-4 reviewer wanted Sompo Japan transfer context surfaced. PR #448 added `previousInsurer: string | null` as a top-level field on `EXTRACTION_JSON_SCHEMA` (`shared/extraction-schema.ts`), `ExtractedPolicyData`, `AnalyzedPolicy`, and propagated through `policy-converter.ts`. Per gotcha #47, OpenAI strict mode requires every property be in `required[]` — the count assertion in `shared/__tests__/extraction-schema.test.ts:24` is now `=== 35` (was 34). Migration 057 instructs the LLM to populate the field from `yenilenmiştir`/`geçiş`/`devir`/`carry-over` patterns. UI consumes it in `PolicyKeyMetricsAndDiscounts.tsx` to append "— preserved from <insurer>" to the NCD discount label when both `ncdDiscount` and `previousInsurer` are set. `generateInsurerTransferInsight()` in `insights.ts` was upgraded to a 2-tier detection: Tier 1 names the insurer when the field is populated; Tier 2 falls back to text-pattern matching when it isn't (defensive for old-prompt extractions).

159. **`app_settings` Cache TTL Is 5 Minutes — Plan Deploy Cycles Accordingly (Added May 2, 2026)**: `ConfigurationService` (`src/lib/config/configuration-service.ts`) caches `app_settings` values for 5 minutes. SQL updates to `app_settings` propagate to in-flight requests on TWO conditions: (a) wait 5+ minutes after the UPDATE before triggering work that should see the new value, OR (b) cycle the Railway production process (one-click redeploy) — fresh process = empty cache. This bit us during Test B: pinning `ai.temperature` to 0 via SQL didn't take effect for ~3 minutes. Workaround for time-sensitive operator changes: SQL UPDATE then immediately trigger a Railway redeploy (any commit on main, or the Railway dashboard "Redeploy" button). Test scripts that depend on a freshly-changed config should either wait 5+ min or document the race in the README.

160. **Coverage-Map Conservative Enrichment Pattern (Added May 2, 2026)**: PR-S2.5, PR-S3.1, PR-S3.4 each added a coverage-name-pattern-based enrichment to the `data.coverages.map(...)` step in `policy-converter.ts:209-231` (currently three rules: `recategorizeIfGlassRepair`, `recoverWrongFuelLimit`, `generateAnadoluHizmetGloss`). All three follow the same conservative pattern: (a) match a specific name regex (e.g. `/cam.*onar[ıi]m/i`), (b) defer to the LLM if the LLM already populated the relevant field, (c) only override/populate when the LLM left the field empty. This pattern is **safe to extend** — adding new enrichment rules doesn't risk overwriting LLM extractions. When adding a new rule, follow the same shape: `function recoverXFromY(name, nameTr, ...evidence): TypedReturn | null` returning `null` to mean "no change". Wire into the map at line 209-231. Add unit tests in `src/lib/ai/__tests__/<rule-name>.test.ts` covering: matching pattern → applies, non-matching pattern → null, LLM-already-populated → null. Gotcha #160 documents the pattern; gotcha #105 documents how the SAME enrichment philosophy applies in `pilot-batch-ingest.ts` (which has its own copy of `extractVehicleInfoFromText` for the same reasons).

161. **Migration Prompt-Bloat Regression (Added May 3, 2026)**: PR #459's migration 058 added a ~3KB AS+ AUTHORIZED SERVICE NETWORK EXTRACTION section to `prompt_templates.user_prompt`. Production verification on the Tiguan fixture showed two regressions: (a) coverage list dropped from 26-29 → 12 rows (LLM truncating output to fit 32K stream-token budget per gotcha #128), (b) "All AI providers failed" cascades on subsequent extractions (combined system-prompt + 60K Tiguan input overflowed input-token budget on both Anthropic and OpenAI). Same-session rollback: migration 059 used `REGEXP_REPLACE` to strip the bloated section; migration 060 re-added the same instruction in ~500 bytes (single paragraph, no worked example, no redundant trigger restatement). **Rule**: prompt additions to the live `prompt_templates.user_prompt` should target ≤500 bytes per migration. Worked examples and trigger-list redundancy are bytes that compete with the document for the model's output budget. If a section needs >500 bytes, justify in the migration comment why the size is necessary AND test against the largest fixture in `tests/fixtures/kasko/` (currently the 60K Tiguan).

162. **`pdf-lib` `PDFDocument.save()` is Non-Deterministic Across Node Processes (Added May 3, 2026)**: Within a single Node process, `pdf-lib`'s `save()` is deterministic — calling it twice on the same loaded PDF produces identical bytes. **Across separate processes**, `save()` produces different bytes for the same source PDF — per-process date / object-ID randomness gets baked into the saved bytes. Confirmed empirically: 3 separate `node` processes loading `tests/fixtures/kasko/anadolu-volkswagen-golf.pdf` and calling `save()` produced 3 different SHA256s. **Implication**: any caching scheme keyed on `sha256(documentBase64)` of post-`pdf-lib` chunks will miss every run because each Node process is a fresh splitter. PR #461's OCR cache hit this exact bug: 13 stale rows piled up, 0 hits, before PR #462 fixed it. **Fix pattern**: cache by source-file SHA + chunk index (e.g. `${sha256(sourceFileBytes)}:${chunkIdx}/${totalChunks}`) rather than chunk bytes. The source bytes ARE byte-stable across processes; pdf-lib's output isn't.

163. **OCR Cache Contract — `cacheKey` Request Field (Added May 3, 2026)**: `POST /api/ai/ocr/document-ai` accepts an optional `cacheKey?: string` field (validated via `documentAISchema` in `server/middleware/validation.ts:140`, max 256 chars). When present, the route hashes the `cacheKey` string for cache lookup; when absent, it falls back to `sha256(documentBase64)`. **Always pass `cacheKey` from CI / smoke / probe scripts** because of gotcha #162 — `cacheKey` from the source file is the only stable identifier across Node processes. Format: `${sha256(sourceFileBytes)}:${chunkIdx}/${totalChunks}`. The cache table is `public.ocr_cache` (migration 061), keyed on `sha256` (which is `sha256(cacheKey)` when provided), with service-role-only RLS following the migration 041 pattern. Cached fields: `text`, `page_count`, `confidence`. NOT cached: `formFields`, `tables` (admin-debug only). On cache hit the response shape includes `cached: true`. Cache lookup failure NEVER blocks the live OCR path; store is fire-and-forget. Verified working in production: `SELECT count(*) FILTER (WHERE last_hit_at IS NOT NULL) FROM ocr_cache` after the second smoke run shows 7/7 hits. **Cache-key-format change cleanup**: when the `cacheKey` derivation rule changes (as PR #462 did from `sha256(documentBase64)` to `sha256(cacheKey)`), the existing rows become unreachable under the new format and accumulate as dead weight. Run `DELETE FROM public.ocr_cache;` in Supabase SQL Editor as part of the rollout to clear them — same pattern applies to any future cache-key migration.

164. **Extraction Model Allowlist on `/api/ai/extract*` (Added May 3, 2026)**: PR #461 added `EXTRACTION_MODEL_ALLOWLIST` constant + `assertExtractionModelAllowed(model, res)` helper at the top of `server/routes/ai/extraction.ts`. Wired into all 3 extract entry points (`/openai`, `/anthropic`, unified `/extract`) immediately after the `model` field is destructured from `req.body`. Rejects unknown model strings with HTTP 400 + `code: 'MODEL_NOT_ALLOWED'`. Current allowlist: `claude-haiku-4-5`, `claude-haiku-4-5-20251001`, `claude-sonnet-4-6`, `claude-sonnet-4-6-20251022`, `gpt-5.4`, `gpt-4o`, `gpt-4o-mini`. The dated-suffix variants are tolerated because Anthropic SDK echoes them in `response.model` (gotcha #151). **Rule**: when adding a new model to the platform (e.g. `claude-opus-4-7` for higher-quality extraction), add its bare alias to the allowlist Set. Without this guard, untrusted callers could pick `claude-3-opus` ($75/M output tokens) and run up the bill — that's why PR #461 introduced it as part of the cost-control work.

165. **Option-A Unverified-Gate Relaxation — Share/Download Always Allowed (Added May 3, 2026)**: PR #456 reversed parts of gotchas #24 (Mar 27 draft-export gating) and #101 (Apr 24 `isUnverified` suppression). The orange `UNVERIFIED — AI output has not yet received human expert verification` banner in `PolicyDetailView.tsx` STILL renders when extraction is incomplete (banner controlled by `isUnverified` flag, line 117). Scoring-card suppressions (`ScoreBreakdown`, `Recommendations`, `PolicyScenariosSection`, `MarketComparison`, `ActuarialInsights`) STILL gate on `isUnverified`. **Only the share + download paths are unconditional now** — the user can share a link or export PDF/CSV/Excel/Text regardless of verification status. The half-gate model (banner says "unverified" but lets user act on it) is the explicit chosen tradeoff. Rationale: hiding the export greyed-out icons created friction without measurably preventing reviewer-quality issues; the banner above the page is enough warning. **Production code changes** are in `src/components/PolicyDetailView.tsx` (banner controlled by `isUnverified`, `draftExportBlocked()` callback removed), `src/components/PolicyDetailView/PolicyExportMenu.tsx` (`isUnverified` + `draftExportBlocked` props dropped, `disabled` state on the trigger removed), and `src/components/PolicyDetailView/PolicyMetadataHeader.tsx` (sharing-disabled toast guard removed, dimmed-button styling removed). Three test files were updated to the new contract: `TrustworthinessUI.test.tsx` (6 cases flipped from "blocks share/export" to "renders banner only"), `draft-gating.test.tsx` (export-blocking section trimmed). When adding new export paths or share mechanisms, do NOT add `isUnverified` checks back — the contract is intentionally "banner + functional".

166. **Diagnosing "All AI providers failed" Cascades (Added May 3, 2026)**: The `All AI providers failed` error appears in extraction logs when both the primary (Anthropic) AND fallback (OpenAI) provider error on the same call. There are three distinct root causes, each with a different fix; the cascade message strips the per-provider detail so you must check Railway logs for the underlying error code from each provider's call site. **(a) Prompt-bloat overflow** (gotcha #161 / PR #459 → #460): a recent migration appended too much content to `prompt_templates.user_prompt`. Combined system-prompt + large policy input overflows the model's input-token budget on both providers at once. Diagnostic: `SELECT name, LENGTH(system_prompt) FROM prompt_templates WHERE name = 'Kasko Extraction'` recently grew. Fix: REGEXP_REPLACE rollback + compact re-add (≤500 bytes). **(b) Provider quota / billing exhaustion** (May-3 session experienced this): Anthropic credit balance ran out AND OpenAI per-day budget cap hit AND/OR Google Document AI quota exceeded. Diagnostic: Anthropic returns 400 `BILLING_ERROR`, OpenAI returns 429 `rate_limit_exceeded`, Document AI returns 429 with quota-project info. Fix: top up the affected provider(s); production extraction recovers automatically. **(c) Genuine service outage** (rare): both providers' status pages would show incidents. Diagnostic: check status.anthropic.com + status.openai.com. Fix: wait or notify users. **Diagnostic flow when this fires**: (1) check Railway logs for the underlying error code from each provider's call site, (2) compare against system-prompt size if migrations recently landed, (3) check provider billing dashboards. Don't roll back prompt migrations until cause (b) is ruled out — May-3 session almost rolled back PR #459 prematurely until quotas-strained-on-OpenAI/Google was identified as the parallel cause.

167. **NodeNext vs Bundler Resolution — `tsc --noEmit` Gives False-Clean Signals (Added May 4, 2026)**: The project has two tsconfig files with different `moduleResolution` settings: `tsconfig.json` (root, used by Vitest and the Vite client build) uses `moduleResolution: "bundler"`, which allows extensionless relative imports. `server/tsconfig.json` uses `moduleResolution: "NodeNext"` (or `node16`), which requires explicit `.js` extensions on all relative imports from ESM modules. Running `npx tsc --noEmit` (or `npm run typecheck`) at the project root **only validates the root tsconfig** and **does not follow project references** to `server/tsconfig.json`. Code that contains extensionless relative imports like `from './base-adapter'` will pass `tsc --noEmit` at root but fail `npm run build:server` with TS2835. This exact gap masked build failures across three sessions — commits in `src/lib/policy-pipeline/` looked type-clean from the root check but caused Railway to reject the production build. **Fix**: always run `npm run build:server` (or `npm run build`, which runs `tsc -b` and follows project references) before committing any `.ts` change. If you only want type-checking without artifact output, `tsc -b --noEmit` follows project references and is the correct substitute. `npm run typecheck` alone is never sufficient for production correctness. Applies to: any file in `src/lib/` that is transitively imported by server code.

168. **Dynamic `import()` Type Annotations Also Require `.js` Extensions Under NodeNext (Added May 4, 2026)**: When a TypeScript file uses a type-only dynamic import in a return type annotation — e.g. `): import('./base-adapter').RequiredCoverageDefinition[]` — this is syntactically distinct from a static `import` statement and will be missed by `grep`-based audits that search for `from '...'` patterns. NodeNext module resolution requires the `.js` extension on this form too: `): import('./base-adapter.js').RequiredCoverageDefinition[]`. The May-4 build audit used `grep -rn "from '\(\.\./\|\./\)'"` to find 11 extensionless imports across 6 files, but the dynamic import type annotation on `adapter-factory.ts:18` was not captured by that pattern and only surfaced when `npm run build:server` was run after the static fixes. **Rule**: treat the build's error output as the authoritative scope for import-extension fixes; `grep` audits are starting points only. Also applies to `export * from './module'` re-exports and `import type` declarations using dynamic path forms.


169. **`npm run build` Prints a Vite Warning About `NODE_ENV=production` in `.env.production` — Expected, Not an Error (Added May 4, 2026)**: Every `npm run build` run prints: `NODE_ENV=production is not supported in the .env file. Only NODE_ENV=development is supported...`. This is a **Vite 7 advisory warning only** — the build exits 0 and the artefacts are correct. Root cause: `.env.production` contains `NODE_ENV=production` (last changed in pre-session commit `a6e4866`). Vite 7 changed how it handles `NODE_ENV` in env files and emits this warning when it detects a non-`development` value. **Do not chase this warning** — it is cosmetic, pre-existing, and fixing it requires moving `NODE_ENV` from `.env.production` into `vite.config.ts`, which carries non-zero risk for zero functional gain. It would only need to be addressed if a future Vite version promotes the warning to an error.
...
- **Production Readiness**: 10/10 (17,469+ tests across the entire project pass cleanly, 0 lint errors, 0 TS errors, 100% test pass rate, PWA support, server hardening, Dynamic AI Rules, DB AI Prompts, UI Evidence Tracking, Safety Governance, PWA icons, 70-policy calibrated grade thresholds, **v5 extraction-depth pass complete (Gemini 2.5 Flash SDK integration, `typeof` type guards, IMM & sigortaBedeli extraction rules, exclusion trigram Jaccard dedup, premium Net/BSMV split, free trial bottleneck removal)**)
- **Last Updated**: May 4, 2026 (2 commits this session on branch `insurgemini202605041923`: `5165a0a` fix(build) — add .js extensions to relative imports in policy-pipeline across 6 files (11 static + 1 dynamic import() type annotation), resolving TS2835 Railway build failure; `1dcf14f` docs(verification) — codify npm run build:server as the required pre-commit TypeScript check in .cursorrules and CLAUDE.md, replacing the insufficient `tsc --noEmit` root-only check. No new migrations. No env var changes. Branch not yet merged — open PR against main.)
