...
describe('AI Routes - OCR & Branch Coverage', () => {
  ...
  describe('OCR endpoint - production error messages', () => {
    ...
    it('returns generic API_NOT_ENABLED message in production', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('PERMISSION_DENIED: Vision API not enabled'))
    ...
    it('returns generic BILLING_ERROR message in production', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('BILLING not enabled'))
    ...
    it('returns generic OCR_FAILED message for unknown errors in production', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('Something completely unexpected'))
    ...
    it('includes details in non-production mode', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('Specific test error'))
  ...
  describe('OCR endpoint - cost tracking', () => {
    ...
    it('handles recordUsage failure in non-production silently', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(JSON.stringify({
          responses: [{ fullTextAnnotation: { text: 'ok', pages: [{ blocks: [{ confidence: 0.9 }] }] } }],
        }), { status: 200 })
      )
  ...
  describe('Document AI - success path with partial data', () => {
    it('handles response with no form fields or tables', async () => {
      process.env = { ...originalEnv, GOOGLE_APPLICATION_CREDENTIALS: '/tmp/creds.json', NODE_ENV: 'test' }
      const app = await createApp()
        ...
        document: {
          ...
          pages: [{
            formFields: [
              ...
              { fieldName: { textAnchor: {} }, fieldValue: { textAnchor: { content: 'some value' } } },
              // Field with empty value - value defaults to ''
              { fieldName: { textAnchor: { content: 'Field Label' } }, fieldValue: { textAnchor: {} } },
              // Field with both empty - should be skipped
              { fieldName: { textAnchor: {} }, fieldValue: { textAnchor: {} } },
              // Field with no bounding poly
              { fieldName: { textAnchor: { content: 'Name' } }, fieldValue: { textAnchor: { content: 'Value' } } },
            ],
              ...
              bodyRows: [
                {
                  cells: [
                    ...
                    { layout: { textAnchor: { content: 'Cell 2' }, confidence: 0.80 } },
                  ],
                },
              ],
            }],
    ...
    it('accepts custom languageHints', async () => {
      ...
      const res = await request(app)
        .post('/api/ai/ocr/document-ai')
        .send({ documentBase64: 'dGVzdA==', mimeType: 'application/pdf', languageHints: ['de', 'en'] })
  ...
  describe('Document AI - production error messages', () => {
    it('returns generic messages in production mode', async () => {
      process.env = { ...originalEnv, GOOGLE_APPLICATION_CREDENTIALS: '/tmp/creds.json', NODE_ENV: 'production' }
      const app = await createApp()

      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(JSON.stringify({ error: { message: 'PERMISSION_DENIED: check service account' } }), { status: 403 })
      )
    ...
    it('returns PAGE_LIMIT_EXCEEDED with user-friendly message in production', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(JSON.stringify({ error: { message: 'Document pages exceed the limit: 15 got 20' } }), { status: 400 })
      )
    ...
    it('returns INVALID_DOCUMENT with generic message in production', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(JSON.stringify({ error: { message: 'INVALID_ARGUMENT: unsupported format' } }), { status: 400 })
      )
    ...
    it('returns generic DOCUMENT_AI_FAILED message for unknown errors in production', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('Random unexpected error'))
  ...
  describe('Diagnose endpoint - OAuth and edge cases', () => {
    it('uses OAuth authentication for Google when service account available', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(JSON.stringify({ responses: [{ fullTextAnnotation: { text: '' } }] }), { status: 200 })
      )
      ...
      expect((fetchOpts.headers as Record<string, string>)['Authorization']).toBe('Bearer test-oauth-token')
      fetchSpy.mockRestore()
    })
    ...
    it('reports HTTP 400 with no errorStatus as INVALID_API_KEY', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(JSON.stringify({ error: { message: 'API key not valid', code: 400 } }), { status: 400 })
      )
    ...
    it('reports sanitized error message in production for Google Vision', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(JSON.stringify({ error: { message: 'PERMISSION_DENIED: API not enabled', status: 'PERMISSION_DENIED' } }), { status: 403 })
      )
  ...
  describe('Unified extract - fallback chain details', () => {
    it('reports billing fallback reason and notifies admin', async () => {
      process.env = { ...originalEnv, OPENAI_API_KEY: 'test-key', ANTHROPIC_API_KEY: 'test-key', NODE_ENV: 'test' }
      const app = await createApp()
      ...
      expect(res.body.fallbackChain[0]).toEqual(expect.objectContaining({
        provider: 'anthropic',
        success: false,
  ...
  describe('Anthropic extract - error classification and notifications', () => {
    it('classifies BILLING_ERROR and notifies admin for billing issues', async () => {
      ...
      expect(res.status).toBe(500)
      expect(res.body.code).toBe('BILLING_ERROR')
      expect(mockNotifyBillingIssue).toHaveBeenCalledWith('Anthropic', expect.any(String), expect.any(Object))
    })
    ...
    it('classifies INVALID_API_KEY and notifies admin for auth errors', async () => {
      ...
      expect(res.status).toBe(500)
      expect(res.body.code).toBe('INVALID_API_KEY')
      expect(mockNotifyAPIError).toHaveBeenCalledWith('Anthropic', 'INVALID_API_KEY', expect.any(String), expect.any(Object))
    })
