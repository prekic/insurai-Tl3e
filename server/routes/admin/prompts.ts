...
router.get('/prompts', authenticateAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (category && typeof category === 'string') {
      templates = await promptService.getPromptsByCategory(category as promptService.PromptCategory)
    } else {
    ...
    log.error('Failed to fetch prompts', {
...
router.get('/prompts/:id', authenticateAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    log.error('Failed to fetch prompt', {
      error: error instanceof Error ? error.message : String(error),
    })
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // Log action
      await logAdminAction(req, 'create', 'prompt_template', template.id, undefined, {
        name,
        category,
      })
 ...
 */
router.get(
  '/prompts/templates/:templateId/versions',
  authenticateAdmin,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      log.error('Failed to get versions', {
        error: error instanceof Error ? error.message : String(error),
      })
 ...
 */
router.post(
  '/prompts/templates/:templateId/rollback/:versionId',
  ...requireSuperAdmin(),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const template = await promptVersioning.rollbackToVersion(
        templateId,
        versionId,
        req.adminUser?.email
      )
      ...
      log.error('Failed to rollback version', {
        error: error instanceof Error ? error.message : String(error),
      })
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      log.error('Failed to list A/B tests', {
        error: error instanceof Error ? error.message : String(error),
      })
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      log.error('Failed to get A/B test', {
        error: error instanceof Error ? error.message : String(error),
      })
    ...
    try {
      ...
      if (!name || !templateId || !controlVersionId || !treatmentVersionIds || !trafficAllocation) {
        res.status(400).json({
          success: false,
          error:
            'name, templateId, controlVersionId, treatmentVersionIds, and trafficAllocation are required',
        })
        return
      }

      // Validate traffic allocation adds up to 100%
      const totalAllocation = Object.values(trafficAllocation as Record<string, number>).reduce(
        (sum, v) => sum + v,
        0
      )
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      log.error('Failed to update A/B test status', {
        error: error instanceof Error ? error.message : String(error),
      })
 ...
 */
router.post(
  '/prompts/ab-tests/:id/apply-winner',
  ...requireSuperAdmin(),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const id = qstr(req.params.id)
      ...
      // Log action
      await logAdminAction(req, 'apply_winner', 'ab_test', id, undefined, {
        winnerId: test.results.winner,
      })

      res.json({ success: true, data: template })
    } catch (error) {
      log.error('Failed to apply A/B test winner', {
        error: error instanceof Error ? error.message : String(error),
      })
...
router.post(
  ...
  (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      log.error('Failed to extract variables', {
        error: error instanceof Error ? error.message : String(error),
      })
