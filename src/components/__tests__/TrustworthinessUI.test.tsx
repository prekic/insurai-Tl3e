...
vi.mock('@/lib/i18n', () => ({
  useI18n: () => mockI18n,
  useTranslation: () => ({ t: EN_TRANSLATIONS, translate: (k: string) => k, locale: 'en', isLoading: false }),
}))
vi.mock('@/lib/i18n/i18n-context', () => ({
  useI18n: () => mockI18n,
  useTranslation: () => ({ t: EN_TRANSLATIONS, translate: (k: string) => k, locale: 'en', isLoading: false }),
  I18nProvider: ({ children }: any) => children,
  useLanguageSelector: () => ({ currentLocale: 'en', locales: [], setLocale: vi.fn(), isLoading: false, progress: { status: 'idle', progress: 100, message: '' } }),
}))
...
describe('Sprint 1: Trustworthiness Hardening UI Tests', () => {
  beforeEach(() => {
    ...
    vi.spyOn(usePolicyEvaluationHook, 'usePolicyEvaluation').mockReturnValue({
      evaluation: {
        ...
        marketComparison: { premiumPercentile: 50, coveragePercentile: 50, competitivePosition: 'average' },
        recommendations: [],
        scenarioCards: []
      },
      isEvaluating: false
  ...
  it('Renders UNVERIFIED banner and blocks export/share when evaluation.isProvisional is true because aiConfidence < 0.85', () => {
    vi.spyOn(useDisplaySafeSummaryHook, 'useDisplaySafeSummary').mockReturnValue({ isDraft: false, isPilotResult: false } as any)
    vi.spyOn(usePolicyEvaluationHook, 'usePolicyEvaluation').mockReturnValue({
      evaluation: { isProvisional: true, grade: 'C', status: 'fair', overallScore: 60, scoreBreakdown: { premium: { score: 60 }, coverage: { score: 60 }, deductible: { score: 60 }, compliance: { score: 60 }, value: { score: 60 } }, recommendations: [], scenarioCards: [] },
      isEvaluating: false
    } as any)

    renderComponent()
    expect(screen.getByText(/UNVERIFIED AI OUTPUT/i)).toBeInTheDocument()
    expect(screen.getByTitle('Export disabled for unverified policies').querySelector('button')!.disabled).toBe(true)
