...
// =============================================================================
// OCR Decision Engine Integration Tests
// =============================================================================

describe('OCR Decision Engine Integration', () => {
  // Helper to set up the OCR Decision Engine mock
  async function setupOCRDecisionMock(action: 'skip_ocr' | 'selective_ocr' | 'full_ocr' = 'skip_ocr') {
    const ocrDecisionEngine = await import('@/lib/ocr-decision/ocr-decision-engine')
    vi.mocked(ocrDecisionEngine.getOCRDecisionEngine).mockReturnValue({
      analyzeDocument: vi.fn(() => ({
        action,
        confidence: action === 'skip_ocr' ? 0.85 : 0.45,
        document_classification: {
          detected_language: { locale_code: 'tr', confidence: 0.9 },
          detected_policy_type: { policy_type_id: 'motor_kasko', policy_type_name: 'Kasko', confidence: 0.85 },
        },
        analysis: {
          text_quality: { quality_score: action === 'skip_ocr' ? 0.8 : 0.3, is_good_quality: action === 'skip_ocr' },
          field_extraction: { extraction_rate: action === 'skip_ocr' ? 0.6 : 0.1 },
          confidence_breakdown: {
            component_scores: {
              char_density: { score: 0.9, weight: 0.25, contribution: 0.225 },
              text_quality: { score: 0.8, weight: 0.30, contribution: 0.24 },
        ...
        pages_to_ocr: action === 'full_ocr' ? [1, 2, 3] : [],
        reasoning: action === 'skip_ocr'
          ? ['High text density detected', 'Turkish language identified', 'Good text quality']
          : ['Low text density', 'Poor text quality', 'Many fields not extractable'],
      })),
      buildDocumentJourneyMetadata: vi.fn((decision: unknown) => ({
        ocr_decision: {
          action: (decision as { action: string }).action,
  ...
  // Helper to set up pdf.js mock
  async function setupPdfParserMock(text = 'Sample policy text with coverage details') {
    const pdfParser = await import('./pdf-parser')
    vi.mocked(pdfParser.extractTextFromPDFWithRetry).mockResolvedValue({
      success: true,
      data: { text, pageCount: 5, metadata: {} },
    })
  }

  // Helper to set up Document AI mock
  async function setupDocumentAIMock(available = true) {
    const documentOcr = await import('./document-ocr')
    vi.mocked(documentOcr.isDocumentOCRAvailable).mockReturnValue(available)
    if (available) {
      vi.mocked(documentOcr.extractWithDocumentAI).mockResolvedValue({
        success: true,
        data: {
          text: 'OCR extracted text from Document AI\nSigorta poliçesi\nTeminat: 500.000 TL',
          pages: [{ pageNumber: 1, text: 'OCR text', confidence: 0.95, warnings: [] }],
          pageCount: 1,
          confidence: 0.95,
          pdfHash: 'mock-hash-abc123',
          formFields: [],
  ...
  // Helper to configure AI as available (required for OCR decision path)
  async function setupAIConfigured() {
    const config = await import('./config')
    vi.mocked(config.isAIConfigured).mockReturnValue(true)
    vi.mocked(config.getConfiguredProviders).mockReturnValue(['openai'])

    // Also need to set up the OpenAI provider response
    const openai = await import('./providers/openai')
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      policyNumber: 'TEST-001',
      provider: 'Test Sigorta',
      policyType: 'kasko',
      insuredName: 'Test Kişi',
      insuredAddress: 'Test Adres',
      startDate: '2025-01-01',
      endDate: '2026-01-01',
      premium: 5000,
      currency: 'TRY',
      paymentFrequency: 'annual',
      coverages: [{ name: 'Collision', nameTr: 'Çarpışma', limit: 100000, deductible: 1000, included: true }],
      specialConditions: [],
      exclusions: [],
      amendmentInfo: { isAmendment: false, amendmentNumber: null, amendmentDate: null, basePolicyNumber: null, amendmentReason: null },
      documentType: 'policy',
      confidenceScores: { overall: 0.85, policyNumber: 0.9, dates: 0.85, premium: 0.8, coverages: 0.8 },
    })
  }
  ...
  beforeEach(async () => {
    ...
    await setupOCRDecisionMock('skip_ocr')
    await setupPdfParserMock()
    await setupDocumentAIMock(true)
  })
  ...
  it('should include OCR decision in extraction result when text is extracted', async () => {
    await setupAIConfigured()
    await setupOCRDecisionMock('skip_ocr')
    await setupPdfParserMock('Sample policy with Turkish text sigorta poliçesi teminat')
    ...
    expect(result.success).toBe(true)
    if (result.success) {
      // The OCR decision should be included in the result
      expect(result.ocrDecision).toBeDefined()
      if (result.ocrDecision) {
        expect(result.ocrDecision.action).toBe('skip_ocr')
        expect(result.ocrDecision.confidence).toBeGreaterThan(0)
        expect(result.ocrDecision.detectedLanguage).toBe('tr')
        expect(result.ocrDecision.detectedPolicyType).toBe('motor_kasko')
        expect(result.ocrDecision.reasoning).toBeInstanceOf(Array)
      }
    }
  })

  it('should skip Document AI OCR when decision is skip_ocr', async () => {
    await setupOCRDecisionMock('skip_ocr')
    await setupPdfParserMock('High quality text with good content')

    const documentOcr = await import('./document-ocr')

    const file = createMockFile('high-quality.pdf', 'application/pdf')

    await extractPolicyFromDocument(file)

    // When skip_ocr, Document AI should NOT be called
    // Since pdf.js extracts text successfully and OCR decision says skip_ocr
    expect(vi.mocked(documentOcr.extractWithDocumentAI)).not.toHaveBeenCalled()
  })

  it('should call Document AI OCR when decision is full_ocr', async () => {
    await setupAIConfigured()
    await setupOCRDecisionMock('full_ocr')
    await setupPdfParserMock('Low quality text that needs OCR')
    await setupDocumentAIMock(true)

    const documentOcr = await import('./document-ocr')

    const file = createMockFile('scanned-document.pdf', 'application/pdf')

    await extractPolicyFromDocument(file)

    // When full_ocr, Document AI should be called
    expect(vi.mocked(documentOcr.extractWithDocumentAI)).toHaveBeenCalled()
  })

  it('should handle OCR Decision Engine errors gracefully', async () => {
    const ocrDecisionEngine = await import('@/lib/ocr-decision/ocr-decision-engine')

    // Mock OCR Decision Engine to throw an error
    vi.mocked(ocrDecisionEngine.getOCRDecisionEngine).mockReturnValue({
      analyzeDocument: vi.fn(() => {
        throw new Error('OCR Decision Engine failed')
      }),
      buildDocumentJourneyMetadata: vi.fn(),
    })

    await setupDocumentAIMock(true)
    await setupPdfParserMock('Some policy text')
    ...
    // Should still succeed (using Document AI as fallback when OCR decision fails)
    expect(result.success).toBe(true)
  })

  it('should use pdf.js text when Document AI is not available and decision recommends OCR', async () => {
    await setupOCRDecisionMock('full_ocr')
    await setupPdfParserMock('Some policy text from pdf.js even though quality is low')
    await setupDocumentAIMock(false) // Document AI NOT available
    ...
    // Should succeed using pdf.js text as fallback even when OCR decision recommends OCR
    expect(result.success).toBe(true)
    // Document AI should not have been called since it's not available
    expect(vi.mocked(documentOcr.extractWithDocumentAI)).not.toHaveBeenCalled()
  })
})
