...
describe('Document AI Fallback to pdf.js', () => {
  ...
  it('should fall back to pdf.js when Document AI fails', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.75, policyNumber: 0.8, provider: 0.7, dates: 0.8, premium: 0.7, coverages: 0.7 },
    })
...
describe('Tiered Confidence System', () => {
  ...
  it('should return LOW_CONFIDENCE error when below minConfidence and no fallback', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.2, policyNumber: 0.1, provider: 0.1, dates: 0.1, premium: 0.1, coverages: 0.1 },
    })
  ...
  it('should set lowConfidence flag when between min and warning thresholds', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.5, policyNumber: 0.5, provider: 0.5, dates: 0.5, premium: 0.5, coverages: 0.5 },
    })
...
describe('Provider Selection', () => {
  ...
  it('should use Claude when anthropic is the primary provider', async () => {
    ...
    vi.mocked(claude.extractWithClaude).mockResolvedValue({
      ...
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.85, policyNumber: 0.9, provider: 0.85, dates: 0.85, premium: 0.8, coverages: 0.8 },
    })
...
describe('Coverage Processing', () => {
  ...
  it('should handle kasko policy with market value coverage', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      paymentFrequency: 'annual',
      coverages: [
        { name: 'Araç Bedeli', limit: 500000, deductible: 0, description: 'Vehicle value', isMarketValue: true, category: 'main' },
        { name: 'Glass Coverage', limit: 25000, deductible: 0, description: '', category: 'supplementary' },
      ],
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.9, policyNumber: 0.95, provider: 0.9, dates: 0.9, premium: 0.85, coverages: 0.85 },
  ...
  it('should handle traffic policy with bodily injury coverage', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      paymentFrequency: null,
      coverages: [
        { name: 'Bedeni Hasar', limit: 2700000, deductible: 0, description: 'Bodily injury', category: 'main' },
        { name: 'Maddi Hasar', limit: 600000, deductible: 0, description: 'Material damage', category: 'main' },
      ],
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.85, policyNumber: 0.9, provider: 0.85, dates: 0.85, premium: 0.8, coverages: 0.8 },
  ...
  it('should handle unknown policy type by defaulting to home', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      currency: 'TRY',
      paymentFrequency: null,
      coverages: [{ name: 'General', limit: 100000, deductible: 0, description: '', category: 'main' }],
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.7, policyNumber: 0.8, provider: 0.7, dates: 0.7, premium: 0.6, coverages: 0.6 },
    })
  ...
  it('should handle null coverages array', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      confidence: { overall: 0.7, policyNumber: 0.7, provider: 0.7, dates: 0.7, premium: 0.7, coverages: 0.7 },
    })
  ...
  it('should use coverage description as fallback name when name is missing', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      paymentFrequency: null,
      coverages: [
        { name: null as unknown as string, limit: 100000, deductible: 0, description: 'Fire protection coverage' },
      ],
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.8, policyNumber: 0.8, provider: 0.8, dates: 0.8, premium: 0.8, coverages: 0.8 },
  ...
  it('should handle coverages with special flags (unlimited, market value)', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      paymentFrequency: null,
      coverages: [
        { name: 'Liability', limit: 0, deductible: 0, description: '', isUnlimited: true, category: 'liability' },
        { name: 'Vehicle Value', limit: 300000, deductible: 0, description: '', isMarketValue: true, category: 'main' },
        { name: 'Roadside Assistance', limit: 5000, deductible: 0, description: '', category: 'assistance' },
      ],
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.85, policyNumber: 0.9, provider: 0.85, dates: 0.85, premium: 0.8, coverages: 0.8 },
...
describe('AI Insights Generation', () => {
  ...
  it('should generate "Standard coverage" when no special features', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      paymentFrequency: null,
      coverages: [
        { name: 'Basic', limit: 100000, deductible: 500, description: '' },
      ],
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.8, policyNumber: 0.8, provider: 0.8, dates: 0.8, premium: 0.8, coverages: 0.8 },
  ...
  it('should generate gap warnings for excessive exclusions', async () => {
    ...
    expect(result.success).toBe(true)
    if (result.success) {
      const hasExclusionWarning = result.policy.aiInsights.some(i => i.includes('Multiple exclusions'))
...
describe('Fallback with Partial Data', () => {
  ...
  it('should use partial data confidence in fallback result', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.15, policyNumber: 0.1, provider: 0.05, dates: 0.1, premium: 0.1, coverages: 0.1 },
    })
...
describe('Coverage Name Translation', () => {
  ...
  it('should use AI-provided nameTr when different from name', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      paymentFrequency: null,
      coverages: [
        { name: 'Fire Coverage', nameTr: 'Yangın Teminatı', limit: 500000, deductible: 0, description: '' },
      ],
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.85, policyNumber: 0.9, provider: 0.85, dates: 0.85, premium: 0.8, coverages: 0.8 },
  ...
  it('should use canonical map when AI nameTr matches name', async () => {
    ...
    vi.mocked(openai.extractWithOpenAI).mockResolvedValue({
      ...
      paymentFrequency: null,
      coverages: [
        { name: 'Fire', nameTr: 'Fire', limit: 500000, deductible: 0, description: '' },
      ],
      specialConditions: [],
      exclusions: [],
      confidence: { overall: 0.85, policyNumber: 0.9, provider: 0.85, dates: 0.85, premium: 0.8, coverages: 0.8 },
...
describe('Risk and Gap Analysis Error Handling', () => {
  ...
  it('should continue without gap analysis when GapDetectionService fails', async () => {
    ...
    vi.mocked(gapDetection.GapDetectionService.analyzePolicy).mockRejectedValue(new Error('Gap detection error'))
