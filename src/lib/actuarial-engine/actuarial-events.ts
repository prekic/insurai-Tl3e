  ...
  try {
    ...
    await adminFetch('/api/admin/actuarial/evaluation-results', {
      ...
      body: JSON.stringify({
        ...
        layerAMs: event.result.layerTimings?.layerA_ms,
        layerBMs: event.result.layerTimings?.layerB_ms,
        layerCMs: event.result.layerTimings?.layerC_ms,
        layerDMs: event.result.layerTimings?.layerD_ms,
