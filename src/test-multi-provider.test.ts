...
describe('multi-provider integration tests (requires PDF fixtures)', () => {
  test('multi-provider extraction', async () => {
    ...
    try {
      ...
      const resultAnthropic = await extractPolicyFromDocument(file, {
      ...
      if (resultOpenAI.success && resultAnthropic.success) {
        ...
        if ((resultOpenAI as any).insights) {
          console.log(
            `OpenAI Insights (Strengths): ${(resultOpenAI as any).insights.strengths?.length}`
          )
        }
        if ((resultAnthropic as any).insights) {
          console.log(
            `Anthropic Insights (Strengths): ${(resultAnthropic as any).insights.strengths?.length}`
          )
        }
      } else {
