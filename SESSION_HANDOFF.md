...
3. **Vitest Mocking Console Errors** — Audit and suppress the remaining non-failing `console.error` logs generated by `vi.mock('./config')` during `extractViaProxy` mock assertions to clean up the test runner output completely.

Full runbook for completed pilot steps: `docs/runbooks/03-pilot-batch-ingestion.md`.

## Current State

**Branch**: `insuraigemini202604120713` — clean. 4 commits ahead of `origin/main` (excluding this doc sync).
**Working tree**: clean.
...
| `src/lib/pdf/noise-stripper.ts` | Removed uncontrolled Regex testing against binary noise streams. |
| `CLAUDE.md` | Added Gotchas #57, #58, #59 and updated completion status. |
...
| 10 | Benchmark premium ranges update | **BLOCKED — needs TSB/SEDDK 2025 market research** |
