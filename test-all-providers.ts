  ...
  try {
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'Extract policy information as JSON.\n\nRespond with valid JSON only.',
        },
    ...
    console.log(
      'OpenAI-mini ends with:',
      content.substring(content.length - 100).replace(/\n/g, ' ')
    )
    try {
      JSON.parse(content)
...
async function testAnthropic(text: string) {
  ...
  try {
    ...
    console.log(
      'Anthropic ends with:',
      jsonContent.substring(jsonContent.length - 100).replace(/\n/g, ' ')
    )
    try {
...
async function run() {
  const text = await extractTextWithPdfjs(
    path.join(process.cwd(), 'upload/real-kasko-pdf/KASKO POLİÇESİ.pdf')
  )
